...
public class BloatListener implements SearchListener {
	...
	/* (non-Javadoc)
	 * @see de.unisb.cs.st.ga.SearchListener#fitnessEvaluation(de.unisb.cs.st.ga.Chromosome)
	 */
	@Override
	...
	public void iteration(List<Chromosome> population) {
		...
		int statements = MaxStatementsStoppingCondition.getNumExecutedStatements();
	...
	public void searchFinished(List<Chromosome> population) {
		...
		try {
			...
			String filename = Properties.getPropertyOrDefault("report_dir", "report")+"/"+Properties.TARGET_CLASS+"_branch_"+num_branch+"_"+Randomness.getInstance().getSeed()+"_";
			filename += xover+"_"+Properties.getPropertyOrDefault("check_rank_length", "true")+"_"+Properties.getPropertyOrDefault("check_parents_length", "true")+"_"+Properties.getPropertyOrDefault("check_best_length", "true")+"_"+Properties.getPropertyOrDefault("check_max_length", "true")+"_"+Properties.CHROMOSOME_LENGTH+".csv";
			...
			for(int num = 0; num < length_history.size(); num++) {
				w.write(num+","+statement_history.get(num)+","+budget_history.get(num)+","+length_history.get(num)+","+fitness_history.get(num)+","+best_length_history.get(num)+","+best_fitness_history.get(num));
				w.write("\n");
			}
			w.close();
		} catch(IOException e) {
	...
	/* (non-Javadoc)
	 * @see de.unisb.cs.st.ga.SearchListener#searchStarted(de.unisb.cs.st.ga.FitnessFunction)
	 */
	@Override
