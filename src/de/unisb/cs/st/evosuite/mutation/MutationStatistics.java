...
public class MutationStatistics implements SearchListener {
	...
	private Logger logger = Logger.getLogger(MutationStatistics.class);
	...
	private void checkEntry(Mutation m) {
		if(!statistics.containsKey(m.getId())) 
			statistics.put(m.getId(), new StatisticEntry());	
		if(m.isKilled())
			statistics.get(m.getId()).is_killed = true;
	}
	
	public void setUnreachable(Mutation m) {
		checkEntry(m);
		statistics.get(m.getId()).unreachable = true;
	}

	public void setAsserted(Mutation m) {
		checkEntry(m);
		statistics.get(m.getId()).has_assertion = true;
	}
	
	public void setKilled(Mutation m) {
		checkEntry(m);
		statistics.get(m.getId()).is_killed = true;
	}
	
	public boolean isKilled(Mutation m) {
		checkEntry(m);
		return statistics.get(m.getId()).is_killed;
	}
	
	public boolean isAsserted(Mutation m) {
		checkEntry(m);
		return statistics.get(m.getId()).has_assertion;
	}

	public boolean hasException(Mutation m) {
		checkEntry(m);
		return statistics.get(m.getId()).has_exception;
	}

*/

	/*
	public void setException(Mutation m, Throwable t) {
		checkEntry(m);
	...
	*/
	...
	public int getNumFunctionExecutedButNotMutation() {
		int num = 0;
		for(StatisticEntry entry : statistics.values()) {
			if(entry.function_executed && !entry.mutant_executed)
				num++;
		}
		return num;
	}
	...
	public int getNumExecuted() {
		int num = 0;
		for(StatisticEntry entry : statistics.values()) {
			if(entry.mutant_executed)
				num++;
		}
		return num;
	}

	public void setMutantExecuted(Mutation m) {
		checkEntry(m);
		statistics.get(m.getId()).mutant_executed = true;
	}

	public void setFunctionExecuted(Mutation m) {
		checkEntry(m);
		statistics.get(m.getId()).function_executed = true;
	}

	public void setFitness(Mutation m, double fitness) {
		checkEntry(m);
		statistics.get(m.getId()).fitness = fitness;
		if(fitness > 0.33)
			setFunctionExecuted(m);
		if(fitness > 0.66)
			setMutantExecuted(m);

	}

	public void addAssertion(Mutation m, Assertion a) {
		//checkEntry(m);
		setAsserted(m);
		if(a instanceof PrimitiveAssertion)
			statistics.get(m.getId()).primitive_assertions++;
		else if(a instanceof CompareAssertion)
			statistics.get(m.getId()).compare_assertions++;
		else if(a instanceof StringAssertion)
			statistics.get(m.getId()).string_assertions++;
		else if(a instanceof InspectorAssertion)
			statistics.get(m.getId()).inspector_assertions++;
	}
	
	public void printStatistics() {
		for(Entry<Long,StatisticEntry> entry : statistics.entrySet()) {
			System.out.println("Mutation "+entry.getKey());
			//.getId()+": "+entry.getKey().getClassName()+"."+entry.getKey().getMethodName()+":"+entry.getKey().getLineNumber());
			...
			if(entry.getValue().exception != null)
				System.out.println("  Exception raised: "+entry.getValue().exception.getMessage());
			else
				System.out.println("  No exception raised");
			System.out.println("  Function executed: "+entry.getValue().function_executed);
			System.out.println("  Mutant executed: "+entry.getValue().mutant_executed);
			System.out.println("  Primitive assertions: "+entry.getValue().primitive_assertions);
			System.out.println("  Compare assertions: "+entry.getValue().compare_assertions);
			System.out.println("  String assertions: "+entry.getValue().string_assertions);
			System.out.println("  Inspector assertions: "+entry.getValue().inspector_assertions);
		}
	}
	...
	public void logStatistics() {
		...
		for(Entry<Long,StatisticEntry> entry : statistics.entrySet()) {
			if(entry.getValue().is_killed)
				continue;
			//int exception = entry.getValue().exception != null ? 1 : 0;
			logger.info("Mutation "+entry.getKey()+"," //: "+entry.getKey().getClassName()+"."+entry.getKey().getMethodName()+":"+entry.getKey().getLineNumber()+","
					+entry.getValue().fitness+","
					+entry.getValue().unreachable+","
					+entry.getValue().function_executed +","
					+entry.getValue().mutant_executed+","
					+entry.getValue().has_exception);
	...
	*/
	...
	protected String writeIntegerChart(List<Integer> values, Long id, String title) {
		...
		GNUPlotTerminal terminal = new FileTerminal("png", REPORT_DIR+"/statistics_"+title+"_"+id+".png");
	...
	protected String writeRunPage(StatisticEntry run) {
		...
		sb.append("<li>Line: <a href=\"#"+run.mutation.getLineNumber()+"\">"+run.mutation.getLineNumber()+"</a>\n");
		...
		for (String line : source) {
			if(linecount >= line_num && linecount < (line_num + 6)) {
				sb.append(String.format(
						"<span class=\"nocode\">%3d: </span>", linecount));
				sb.append(StringEscapeUtils.escapeHtml(line));
				sb.append("\n");
			}
			linecount++;
		...
		if(run.fitness_history.isEmpty()) {
			...
			String filename = writeDoubleChart(run.fitness_history, run.mutation.getId(), "Fitness");
		...
		if(run.length_history.isEmpty()) {
			...
			String filename = writeIntegerChart(run.length_history, run.mutation.getId(), "Length");
	...
	protected void writeRunTable(StringBuffer buffer) {
		...
		for(StatisticEntry entry : statistics) {
			...
			buffer.append("<td>"+entry.mutation.getClassName()+"."+entry.mutation.getMethodName()+":"+entry.mutation.getLineNumber()+"</td>");
			buffer.append("<td>");
	...
	public void writeReport() {
		...
		report.append("<h1>muTest Report for "+ MutationProperties.PROJECT_PREFIX +"</h1>\n");
//		report.append("<h2>Run on "+System.getenv("USERNAME")+"@"+System.getenv("HOSTNAME")+"</h2>\n");
	...
	public void searchStarted(FitnessFunction objective) {
		...
		if(objective instanceof TestImpactFitness) {
			TestImpactFitness fitness = (TestImpactFitness)objective;
			entry.mutation = fitness.current_mutation;
		} else {
			entry.mutation = null;
		}
