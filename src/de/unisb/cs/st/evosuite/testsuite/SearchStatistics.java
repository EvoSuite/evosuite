...
public class SearchStatistics implements SearchListener {
	...
	private Logger logger = Logger.getLogger(SearchStatistics.class);
	
	private static final File REPORT_DIR = new File(Properties.getPropertyOrDefault("report_dir", "report"));
	...
	private List<StatisticEntry> statistics = new ArrayList<StatisticEntry>();
	...
	protected String writeIntegerChart(List<Integer> values, String className, String title) {
		File file = new File(REPORT_DIR.getAbsolutePath()+"/img/statistics_"+title+"_"+className+".png");
		JavaPlot plot = new JavaPlot();
		GNUPlotTerminal terminal = new FileTerminal("png", REPORT_DIR+"/img/statistics_"+title+"_"+className+".png");
	...
	private int getNumber(final String className) {
		...
		FilenameFilter filter = new FilenameFilter() {
		    public boolean accept(File dir, String name) {
		        return name.startsWith("statistics_"+className) && name.endsWith(".csv"); // && !dir.isDirectory();
		    }
		};
			...
			for(TestCase test : run.tests) {
				...
				/*
				 if(test.exceptionThrown != null) {
					sb.append("<p>Raises:");
					sb.append(test.exceptionThrown);
					sb.append("</p>");
				}
				*/
				...
				for (String line : code.split("\n")) {
					sb.append(String.format(
						...
						linecount, linecount));
					/*
					if(test.exceptionsThrown != null && test.exception_statement == test_line)
						sb.append("<span style=\"background: #FF0000\">");*/
					sb.append(StringEscapeUtils.escapeHtml(line));
					/* if(test.exceptionThrown != null && test.exception_statement == test_line)
						sb.append("</span>"); */
					test_line++;
					linecount++;
					sb.append("\n");
...
//		return file.getAbsolutePath();
	...
	protected void writeResultTable(StringBuffer buffer, StatisticEntry entry) {
		...
		buffer.append("<li>Time for search: "+String.format("%d:%02d:%02d", duration_GA/3600, (duration_GA%3600)/60, (duration_GA%60))+"\n");
		buffer.append("<li>Time for minimization: "+String.format("%d:%02d:%02d", duration_MI/3600, (duration_MI%3600)/60, (duration_MI%60))+"\n");
		buffer.append("<li>Total time: "+String.format("%d:%02d:%02d", duration_TO/3600, (duration_TO%3600)/60, (duration_TO%60))+"\n");
	...
	protected void writeRunTable(StringBuffer buffer) {
		...
		for(StatisticEntry entry : statistics) {
			...
			long duration_TO = (entry.minimized_time - entry.start_time)/1000;
			buffer.append("<td>");
			buffer.append(String.format("%d:%02d:%02d", duration_TO/3600, (duration_TO%3600)/60, (duration_TO%60)));
			buffer.append("</td>");
			buffer.append("<td>");
			buffer.append(String.format("%.2f", (100.0*entry.covered_goals/(1.0*entry.total_goals))));
			buffer.append("%</td>");
	...
	public void writeCSV() {
		...
		try {
			...
			out.write(REPORT_DIR.getAbsolutePath()+"/data/statistics_"+entry.className+"-"+entry.id+".csv\n");
			out.close(); 
		...
		writeCSVData(REPORT_DIR.getAbsolutePath()+"/data/statistics_"+entry.className+"-"+entry.id+".csv", 
...
//			ExecutionTrace trace = test.last_result.trace; //executeTest(test.test, entry.className);
			...
			/*
				...
				trace = executeTest(test.test, entry.className);
				*/
		...
		for(String key : predicate_count.keySet()) {
			...
			if(val == 1 && true_distance.get(key) > 0.0)
				true_distance.put(key, 1.0);
			else if(val == 1 && false_distance.get(key) > 0.0)
