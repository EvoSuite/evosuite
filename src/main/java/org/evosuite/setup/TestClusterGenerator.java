...
public class TestClusterGenerator {
	...
	public static void resetCluster() throws RuntimeException,
			ClassNotFoundException {
		BytecodeInstructionPool.clearAll();
		BranchPool.clear();
		CFGMethodAdapter.methods.clear();
		TestCluster.reset();
		generateCluster(Properties.TARGET_CLASS,
	...
	public static void generateCluster(String targetClass,
		...
		if (Properties.INSTRUMENT_CONTEXT
			...
			for (String callTreeClass : DependencyAnalysis.getCallTree()
					.getClasses()) {
				try {
		...
		for (ClassNode classNode : DependencyAnalysis.getAllClassNodes()) {
			...
			for (MethodNode methodNode : methods) {
				...
				parameterClasses.addAll(Arrays.asList(Type
				...
				while (it.hasNext()) {
					...
					if (insn.getOpcode() == Opcodes.CHECKCAST) {
						...
						invokedClasses.add(Type
								.getObjectType(methodInsnNode.owner));
					}
				}
			}
		}
		...
		/*
		 * for (Type type : castClasses) { addClass(type); }
		 */

		resolveDependencies();
		/*
		 * // TODO: Not sure what to do with these invokedClasses.removeAll(callTreeClasses); invokedClasses.removeAll(castClasses);
		 * invokedClasses.removeAll(parameterClasses);
		 * 
		 * LoggingUtils.getEvoLogger().info("Calltree classes"); for (Type type : callTreeClasses) { LoggingUtils.getEvoLogger().info("  " +
		 * type.getClassName()); }
		 * 
		 * LoggingUtils.getEvoLogger().info("Cast classes"); for (Type type : castClasses) { LoggingUtils.getEvoLogger().info("  " +
		 * type.getClassName()); }
		 * 
		 * LoggingUtils.getEvoLogger().info("Parameter classes"); for (Type type : parameterClasses) { LoggingUtils.getEvoLogger().info("  " +
		 * type.getClassName()); }
		 * 
		 * LoggingUtils.getEvoLogger().info("Invoked classes"); for (Type type : invokedClasses) { LoggingUtils.getEvoLogger().info("  " +
		 * type.getClassName()); }
		 * 
		 * LoggingUtils.getEvoLogger().info("Subclasses"); for (Type type : subClasses) { LoggingUtils.getEvoLogger().info("  " +
		 * type.getClassName()); }
		 * 
		...
		/*
		 * LoggingUtils.getEvoLogger().info("1"); LoggingUtils.getEvoLogger().info(callTree.toString()); LoggingUtils.getEvoLogger().info("2");
		 * LoggingUtils.getEvoLogger().info(inheritanceTree.toString()); LoggingUtils.getEvoLogger().info("3");
		 * LoggingUtils.getEvoLogger().info(inheritanceTree.getSubclasses(Properties.TARGET_CLASS).toString()); LoggingUtils.getEvoLogger().info("4");
	...
	/**
	 * Sort the list of classes by their distance to the target class in terms of package
	 * 
	 * @param targetClass
	 * @param subClasses
	 * @return
	...
	private static void initializeTargetMethods() throws RuntimeException,
		...
		ClassNode targetClassNode = DependencyAnalysis
				.getClassNode(Properties.TARGET_CLASS);
		...
		while (!innerClasses.isEmpty()) {
			...
			try {
				logger.debug("Loading inner class: " + icn.innerName + ", "
				...
				if (!targetClasses.contains(innerClass)) {
					...
					ClassNode innerClassNode = DependencyAnalysis
							.getClassNode(innerClassName);
		...
		for (Class<?> clazz : targetClasses) {
			...
			for (Constructor<?> constructor : getConstructors(clazz)) {
				...
				if (Properties.TT) {
					String orig = name;
					name = BooleanTestabilityTransformation
							.getOriginalNameDesc(
									clazz.getName(),
									"<init>",
									org.objectweb.asm.Type
			...
			for (Method method : getMethods(clazz)) {
				...
				if (Properties.TT) {
					String orig = name;
					name = BooleanTestabilityTransformation
							.getOriginalNameDesc(clazz.getName(), method
									.getName(), org.objectweb.asm.Type
			...
			for (Field field : getFields(clazz)) {
				...
				if (canUse(field)) {
					...
					cluster.addGenerator(
							new GenericClass(field.getGenericType()), field);
		...
		if (Properties.INSTRUMENT_PARENT) {
			for (String superClass : inheritanceTree
					.getSuperclasses(Properties.TARGET_CLASS)) {
				try {
					Class<?> superClazz = TestGenerationContext
							.getClassLoader().loadClass(superClass);
	...
	public static Set<Method> getMethods(Class<?> clazz) {
		...
		try {
			...
			logger.info("Error while trying to load methods of class "
					+ clazz.getName() + ": " + e);
		}
	...
	public static Set<Field> getFields(Class<?> clazz) {
		...
		try {
			...
			// TODO: What shall we do?
			logger.info("Error while trying to load fields of class "
					+ clazz.getName() + ": " + e);
		}
	...
	public static Set<Field> getAccessibleFields(Class<?> clazz) {
		...
		try {
			...
			logger.info("Error while accessing fields of class "
					+ clazz.getName() + " - check allowed permissions: " + t);
		}
		return fields;
	}
	...
	private static void addDependency(Class<?> clazz) {
		...
		logger.debug("Concrete classes for " + clazz.getName() + ": "
				+ actualClasses);
		...
		for (Constructor<?> constructor : getConstructors(clazz)) {
			...
			if (Properties.TT) {
				...
				name = BooleanTestabilityTransformation.getOriginalNameDesc(
						clazz.getName(), "<init>", org.objectweb.asm.Type
								.getConstructorDescriptor(constructor));
				if (!orig.equals(name))
					logger.info("TT name: " + orig + " -> " + name);
	...
	private static Set<Class<?>> getConcreteClasses(Class<?> clazz) {
		...
		if (Modifier.isAbstract(clazz.getModifiers())
			...
			Set<String> subClasses = inheritanceTree.getSubclasses(clazz
					.getName());
