...
import org.evosuite.testcarver.instrument.classloader.SystemClassLoader;
...
//		}
	...
	private final Class<?> getClassForName(String type)
	{
		try 
		{
			...
			{
				return Class.forName("java.lang." + type, true, TestGenerationContext.getClassLoader());
			}
		
			if(type.endsWith("[]"))
			{
				type = type.replace("[]", "");
				return Class.forName("[L" + type + ";", true, TestGenerationContext.getClassLoader());
			}
			else
			{
				return Class.forName(type.replace('/', '.'), true, TestGenerationContext.getClassLoader());
			}
		} 
		catch (final ClassNotFoundException e) 
		{
...
//		}
	...
	public void createCollectionInitStmt(final CaptureLog log, final int logRecNo) 
	{
		try
		{
			...
			final ConstructorStatement constrStmt = new ConstructorStatement(testCase, collType.getConstructor(new Class<?>[0]), collType, noParams);
			...
			for(int i = 0; i < params.length; i++)
			{
				...
				if(argOID == null)
				{
					paramList.add(testCase.addStatement(new NullStatement(testCase, methodParamTypeClasses[i])));
				}
				else
				{
				...
				methodStmt = new MethodStatement(testCase, method, collRef, method.getReturnType(), paramList);
