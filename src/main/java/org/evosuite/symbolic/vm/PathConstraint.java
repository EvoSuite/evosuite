...
public final class PathConstraint {
	...
	public void pushBranchCondition(String className, String methName,
			int branchIndex, IntegerConstraint c) {
	...
	private Constraint<?> removeCMPFormConstraint(IntegerConstraint c) {
		...
		if (c.getLeftOperand() instanceof IntegerComparison) {
			...
			case EQ:
				if (value < 0) {
					return (new IntegerConstraint(cmp.getLeftOperant(),
							Comparator.LT, cmp.getRightOperant()));
				} else if (value == 0) {
					return (new IntegerConstraint(cmp.getLeftOperant(),
							Comparator.EQ, cmp.getRightOperant()));
				} else {
					return (new IntegerConstraint(cmp.getLeftOperant(),
							Comparator.GT, cmp.getRightOperant()));
				}
			case NE:
				if (value < 0) {
					return (new IntegerConstraint(cmp.getLeftOperant(),
							Comparator.GE, cmp.getRightOperant()));
				} else if (value == 0) {
					return (new IntegerConstraint(cmp.getLeftOperant(),
							Comparator.NE, cmp.getRightOperant()));
				} else {
					return (new IntegerConstraint(cmp.getLeftOperant(),
							Comparator.LE, cmp.getRightOperant()));
				}
			case LE:
				if (value < 0) {
					return (new IntegerConstraint(cmp.getLeftOperant(),
			...
			case EQ:
				if (value < 0) {
					return (new RealConstraint(cmp.getLeftOperant(),
							Comparator.LT, cmp.getRightOperant()));
				} else if (value == 0) {
					return (new RealConstraint(cmp.getLeftOperant(),
							Comparator.EQ, cmp.getRightOperant()));
