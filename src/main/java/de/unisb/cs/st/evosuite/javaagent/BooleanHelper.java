...
public class BooleanHelper {
	...
	public static int fromDouble(double d) {
		...
		/*
		 * if (d > Integer.MAX_VALUE) return Integer.MAX_VALUE; else if (d <
		 * Integer.MIN_VALUE) return Integer.MIN_VALUE; else
		 */
		if (d == 0.0) {
			return 0;
		} else {
	...
	public static int getDistance(int original) {
		...
		distanceStack.clear();
		/*
		 * if (l <= 1) { //distance += K; if (original <= 0) distance =
		 * -distance; logger.debug("Distance (2)" + distance); return distance;
		 * }
		 */
	...
	public static int instanceOf(Object o, Class<?> c) {
		...
		// logger.info("Checking whether " + o.getClass().getName() +
		// " can be assigned to "
		// + c.getName());
