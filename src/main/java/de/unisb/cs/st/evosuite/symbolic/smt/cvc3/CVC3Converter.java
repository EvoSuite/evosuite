...
public class CVC3Converter {
	...
	private Expr visit(BinaryExpression<?> expr) {
		...
		case SHR:
			...
			// return cvc3.BvShr(visit(expr.getLeftOperand()),
			// visit(expr.getRightOperand()));
		case SHL:
			throw new RuntimeException("Shift not yet supported");
			// return cvc3.BvShl(visit(expr.getLeftOperand()),
			// visit(expr.getRightOperand()));
		case IXOR:
	...
	private Expr visit(Variable<?> expr) {
		...
		// symbols.add(symbol); // TODO: Distinguish between int and float
		// types!
		return symbol;
	}

	private Expr visitConstraint(Comparator comparator, Expression<?> left, Expression<?> right) {
