...
public class ResourceList {
	...
	private static Collection<String> getResources(final String element, final Pattern pattern) {
		...
		if (file.isDirectory()) {
			try {
				retval.addAll(getResourcesFromDirectory(file, pattern, file.getCanonicalPath()));
			} catch (IOException e) {
	...
	private static Collection<String> getResourcesFromJarFile(final File file, final Pattern pattern) {
		assert (file.exists()) : "We can only get resources from an existing jar. " + file.getAbsolutePath()
				+ " is not existing";
		assert (file.isFile()) : "We can only get resources from a FILE. " + file.getAbsolutePath()
				+ " is apparently not a file";
		final ArrayList<String> retval = new ArrayList<String>();
		ZipFile zf;
		try {
			zf = new ZipFile(file);
