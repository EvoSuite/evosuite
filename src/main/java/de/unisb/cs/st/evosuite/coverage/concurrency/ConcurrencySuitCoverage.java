...
public class ConcurrencySuitCoverage extends TestSuiteFitnessFunction {
	...
	// #TODO currently we only generate schedules, which can be reached in one
	// run. In one method (that depends on the CFG for multi method stuff) If we
	// allow arbitrarily many calls between reaching a scheduling point, what
	// would change?
	...
	public int getDistance(List<SchedulingDecisionTuple> target, List<SchedulingDecisionTuple> seen) {
		assert (seen != null);
		assert (target != null);
	...
	public double getFitness(Chromosome individual) {
		...
		try {
			...
			logger.info("We generated " + goalSchedules.size() + " schedules which should be covered. In : "
		...
		// if(fitness==0.0)System.out.println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
		// + fitness);
	...
	private final Set<SchedulingDecisionList> getNextSteps(final SchedulingDecisionList history,
	 ...
	 * @return
	 */
	private boolean isAfter(SchedulingDecisionTuple tuple, SchedulingDecisionList history,
			final RawControlFlowGraph completeCFG) {
