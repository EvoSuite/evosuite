...
public class ScanProject {
	...
	public final static class FileClassLoader extends ClassLoader {
		...
		protected Class<?> findClass(String name) throws ClassNotFoundException {
			...
			try {
				...
				return defineClass(reader.getClassName().replace("/", "."), out.toByteArray(), 0, out.size());
	...
	public static void main(String[] args) {
		...
		try {
			if (Properties.PROJECT_PREFIX != null) {
				System.out.println("* Analyzing project prefix: " + Properties.PROJECT_PREFIX);
		...
		if (directory.getName().endsWith(".jar")) {
			...
			try {
				...
				System.out.println("Cannot find class "
						+ directory.getName().substring(0, directory.getName().length() - 6) + " in path " + directory);
			} catch (ExceptionInInitializerError e) {
	...
	private static Set<Class<?>> getClasses(String packageName) throws ClassNotFoundException, IOException {
		...
		Collection<String> list = ResourceList.getResources(Pattern.compile(packageName + "/.*\\.class$"));
	...
	private static Set<Class<?>> getClassesJar(File file) throws ClassNotFoundException {
		...
		while (e.hasMoreElements()) {
			...
			try {
				...
				System.out.println("Cannot find class " + file.getName().substring(0, file.getName().length() - 6));
	...
	private static Set<Class<?>> loadClass(File file, String packageName) throws ClassNotFoundException {
		Set<Class<?>> set = new HashSet<Class<?>>();
		if (file.isDirectory()) {
			assert !file.getName().contains(".");
			set.addAll(findClasses(file, packageName + "." + file.getName()));
		} else if (file.getName().endsWith(".class")) {
			System.out.println("    " + file.toString().replace(".class", "").replace("/", "."));
			if (Properties.STUBS) {
