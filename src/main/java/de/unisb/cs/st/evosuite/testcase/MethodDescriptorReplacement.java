...
public class MethodDescriptorReplacement {
	...
	public List<Type> getParameterTypes(Constructor<?> constructor) {
		...
		/*
		 * if (Properties.TRANSFORM_BOOLEAN) { descriptor =
		 * DescriptorMapping.getInstance().getOriginalDescriptor(className,
		 * "<init>", descriptor); }
		 */
		if (hasKey(className, "<init>", descriptor)) {
			...
			/*
			 * if (Properties.TRANSFORM_BOOLEAN) { logger.info("Checking " +
			 * className + ".<init>" + descriptor); int num = 0; for
			 * (org.objectweb.asm.Type asm_param :
			 * DescriptorMapping.getInstance(
			 * ).getOriginalTypes(constructor.getDeclaringClass().getName(),
			 * "<init>",
			 * org.objectweb.asm.Type.getConstructorDescriptor(constructor))) {
			 * logger.info("Next type: " + asm_param); parameters.set(num,
			 * getType(asm_param)); num++; } }
			 */
			constructor_parameters.put(constructor, parameters);
			return parameters;
		}
	...
	public List<Type> getParameterTypes(Type callee, Method method) {
		...
		/*
		 * if (Properties.TRANSFORM_BOOLEAN) { List<Type> parameters = new
		 * ArrayList<Type>(); for (org.objectweb.asm.Type asm_param :
		 * DescriptorMapping
		 * .getInstance().getOriginalTypes(method.getDeclaringClass().getName(),
		 * method.getName(),
		 * org.objectweb.asm.Type.getMethodDescriptor(method))) {
		 * logger.info("Next type: " + asm_param);
		 * parameters.add(getType(asm_param)); } return parameters; }
		 */
		...
		if (hasKey(className, method.getName(), descriptor)) {
			String replacement = descriptors.get(className + "." + method.getName() + descriptor);
			...
			/*
			 * if (Properties.TRANSFORM_BOOLEAN) { logger.info("Checking " +
			 * className + "." + method.getName() + descriptor);
			 * 
			 * int num = 0; for (org.objectweb.asm.Type asm_param :
			 * DescriptorMapping
			 * .getInstance().getOriginalTypes(method.getDeclaringClass
	...
	private void getDescriptorMapping() {
		...
		// File file = new
		// File(MutationProperties.OUTPUT_DIR+"/"+className+".obj");
