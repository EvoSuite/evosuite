...
public class DefUseFitnessCalculations {
	 ...
	 * 
	 */
	public static double calculateAlternativeFitness(Definition goalDefinition, TestChromosome individual,
			ExecutionResult result, ExecutionTrace objectTrace, Integer objectId, Integer usePos,
			List<Integer> goalDefinitionPositions) {

		int lastGoalDUPos = getMaxEntryLowerThan(goalDefinitionPositions, usePos);
		Set<Integer> overwritingDefIds = DefUseExecutionTraceAnalyzer.getOverwritingDefinitionIdsBetween(
				goalDefinition, objectTrace, lastGoalDUPos + 1, usePos, objectId);
	 ...
	 */
	public static double calculateAlternatveFitnessForOverwritingDefinition(TestChromosome individual,
			ExecutionResult result, ExecutionTrace objectTrace, Integer objectId, int overwritingDefId,
		...
		if (overwritingDefinition == null) {
			throw new IllegalStateException(
					"expect DefUsePool to know definitions traced by instrumented code. defId: " + overwritingDefId);
		}
		...
		double newFitness = calculateFitnessForDURange(individual, result, objectTrace, objectId,
				alternativeBranchFitness, overwritingDefinition, false, duCounterStart, duCounterEnd);
		// debugging stuff
		if (DEBUG && (newFitness == 0.0)) {
			...
			ExecutionTrace cutTrace = objectTrace.getTraceInDUCounterRange(overwritingDefinition, false,
	...
	public static double calculateDefFitnessForCompleteTrace(Definition targetDefinition,
			BranchCoverageTestFitness targetFitness, TestChromosome individual, ExecutionResult result) {
	 ...
	 */
	public static double calculateDUFitness(DefUseCoverageTestFitness goal, TestChromosome individual,
		...
		// individual objects
		double defFitness = calculateDefFitnessForCompleteTrace(goalDefinition, goalDefinitionBranchFitness,
				individual, result);
		if (defFitness != 0) {
		...
		for (Integer objectId : objectPool) {
			...
			if (!hasEntriesForId(result.getTrace().passedDefinitions.get(goalVariable), objectId,
	...
	public static double calculateFitnessForDURange(TestChromosome individual, ExecutionResult result,
			ExecutionTrace targetTrace, Integer objectId, BranchCoverageTestFitness targetFitness, DefUse targetDU,
			boolean wantToCoverTargetDU, int duCounterStart, int duCounterEnd) {

		// filter trace
		ExecutionTrace cutTrace = targetTrace.getTraceInDUCounterRange(targetDU, wantToCoverTargetDU, duCounterStart,
				duCounterEnd);
		// calculate fitness
		...
		if ((fitness == 0.0) && wantToCoverTargetDU) {
			...
			throw new IllegalStateException("use cant have fitness 0 in this cut trace: "
					+ cutTrace.toDefUseTraceInformation(targetDU.getDUVariableName(), objectId));
		}
		return fitness;
	}
	 ...
	 */
	public static double calculateFitnessForObject(DefUseCoverageTestFitness goal, TestChromosome individual,
		...
		List<Integer> goalDefinitionPositions = DefUseExecutionTraceAnalyzer.getDefinitionPositions(goalDefinition,
				objectTrace, objectId);
		for (Integer usePos : usePositions) {
			int activeDefId = DefUseExecutionTraceAnalyzer.getActiveDefinitionIdAt(goalVariable, objectTrace, usePos,
					objectId);
		...
		if (goalUse.getControlDependentBranchId() != -1) {
			for (Integer goalDefinitionPos : goalDefinitionPositions) {
				double useFitness = calculateUseFitnessForDefinitionPos(goalDefinition, goalUse, goalUseBranchFitness,
						individual, result, objectTrace, objectId, goalDefinitionPos);
	 ...
	 * special s. isSpecialGoalDefinition()
	 */
	public static double calculateUseFitnessForCompleteTrace(Use targetUse, BranchCoverageTestFitness targetFitness,
	 ...
	 * 
	 */
	public static double calculateUseFitnessForDefinitionPos(Definition targetDefinition, Use targetUse,
			BranchCoverageTestFitness targetUseBranchTestFitness, TestChromosome individual, ExecutionResult result,
	 ...
	 */
	public static BranchCoverageTestFitness getBranchTestFitness(BytecodeInstruction v, boolean targetExpressionValue) {
		...
		if (v.getControlDependentBranchId() == -1) {
			...
			r = new BranchCoverageTestFitness(new BranchCoverageGoal(b, targetExpressionValue, v.getClassName(),
					v.getMethodName()));
		}
		return r;
	}
	...
	public static BranchCoverageTestFitness getRootBranchTestFitness(BytecodeInstruction v) {
		return new BranchCoverageTestFitness(new BranchCoverageGoal(v.getClassName(), v.getClassName() + "."
				+ v.getMethodName()));
	}
	...
	public static boolean hasEntriesForId(Map<Integer, HashMap<Integer, Integer>> objectDUMap, Integer objectId,
			int targetId) {
		if (objectDUMap == null) {
			return false;
		}
	...
	// maybe even make them new constructors for BranchCoverageTestFitness, that
	// seems reasonable
	...
	public static boolean traceCoversGoal(DefUseCoverageTestFitness goal, Chromosome individual, ExecutionTrace trace) {
		...
		for (Integer objectID : objectPool) {
			...
			if (goalDefinition.isStaticDefUse() && goalDefinition.getMethodName().startsWith("<clinit>")) {
				return true;
			}

			for (Integer usePos : usePositions) {

				if (DefUseExecutionTraceAnalyzer.getActiveDefinitionIdAt(goalVariable, trace, usePos, objectID) == goalDefinition
						.getDefId()) {
					return true;
				}
			}
