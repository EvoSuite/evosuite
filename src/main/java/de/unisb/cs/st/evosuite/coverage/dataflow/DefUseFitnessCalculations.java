...
public class DefUseFitnessCalculations {
	...
	public static final boolean ENABLE_ALTERNATIVE_FITNESS_CALCULATION =  
		Properties.getPropertyOrDefault("enable_alternative_fitness_calculation", true);
	...
	static {
		if(Properties.CRITERION.equals("defuse"))
			if(ENABLE_ALTERNATIVE_FITNESS_CALCULATION) {
				System.out.println("* Alternative fitness calculation mode: "+ALTERNATIVE_FITNESS_CALCULATION_MODE);
				if(!ALTERNATIVE_FITNESS_CALCULATION_MODE.equals("sum"))  {
				...
				SINGLE_ALTERNATIVE_FITNESS_RANGE = 1;
					...
					System.out.println("  - Single alternative fitness range: "+SINGLE_ALTERNATIVE_FITNESS_RANGE);
	...
	/**
	 * Calculates the DefinitionUseCoverage fitness for the given DUPair on the given ExecutionResult
	 * 
	...
	public static double calculateDUFitness(DefUseCoverageTestFitness goal, 
		...
		double defFitness = calculateDefFitnessForCompleteTrace(goalDefinition,goalDefinitionBranchFitness,individual, result);
		if (defFitness != 0) {
		...
		for(Integer objectId : objectPool) {
			...
			double newFitness = calculateFitnessForObject(goal, individual,result,objectId);
	...
	public static double calculateFitnessForObject(DefUseCoverageTestFitness goal,
		...
		if(isSpecialDefinition(goalDefinition)) {
			double useFitness = executeBranchFitnessForTrace(individual,result,objectTrace,goalUseBranchFitness);
		...
		for(Integer usePos : usePositions) {
			int activeDefId = DefUseExecutionTraceAnalyzer.getActiveDefinitionIdAt(goalVariable, objectTrace, usePos, objectId);
			if (activeDefId == goalDefinition.getDefId()) {
				...
				if(ENABLE_ALTERNATIVE_FITNESS_CALCULATION) {
					...
					if(hasEntryLowerThan(goalDefinitionPositions, usePos)) {
						...
						double alternativeFitness = calculateAlternativeFitness(goalDefinition, individual, result, 
								objectTrace, objectId, usePos, goalDefinitionPositions);
						if(alternativeFitness <= 0.0 || alternativeFitness>1.0)
							throw new IllegalStateException("alternative fitness expected to be in (0,1]");
		...
		if(goalUse.getBranchId() != -1)
			for(Integer goalDefinitionPos : goalDefinitionPositions) {
				double useFitness = calculateUseFitnessForDefinitionPos(goalDefinition, goalUse, goalUseBranchFitness, individual, result, objectTrace, objectId, goalDefinitionPos);
	 ...
	 * 
	 * Now there are several possibilities how the alternative fitness for all overwriting
	 * definitions can be computed from the alternative fitness for a single overwriting definition.
	 * The mode can be configured by setting ALTERNATIVE_FITNESS_CALCULATION_MODE 
	 * to the respective value noted:
	 * 
	 *  - all singled fitness are summed up and normalized again 	- MODE: "sum"
	 *  	- if you wish not to normalize the sum if 
	 *  		there was only 1 overwriting definition set
	 *  		DONT_NORMALIZE_SINGLE_OVERWRITING_DEFINITION_SUM
	 *  	- in "sum"-mode you can also stretch each individual 
	 *  		overwriting definition fitness by using
	 *  		SINGLE_ALTERNATIVE_FITNESS_RANGE
	 *  - the minimum/maximum over all single fitness is taken 		- MODE: "min"/"max" 
	 *  - the average over all single fitness is taken				- MODE: "avg"
	 *  - only consider traces where there is just one overwriting	- MODE: "single" 
	 *  	definition and return the single alternative fitness
	 *  
	 * Additionally there are two mutually not exclusive ways to penalize multiple overwriting definitions:
	 * 
	 *  - PENALIZE_MULTIPLE_OVERWRITING_DEFINITIONS_FLAT adds i to the i'th single fitness
	 *  - PENALIZE_MULTIPLE_OVERWRITING_DEFINITIONS_LINEARLY multiplies the i'th fitness by i
	 * 
	 * 
	 */
	public static double calculateAlternativeFitness(Definition goalDefinition, 
		...
		Set<Integer> overwritingDefIds = 
			DefUseExecutionTraceAnalyzer.getOverwritingDefinitionIdsBetween(goalDefinition, 
					objectTrace, lastGoalDUPos+1, usePos, objectId);
		...
		if(ALTERNATIVE_FITNESS_CALCULATION_MODE.equals("single") && overwritingDefIds.size()!=1)
			return 1.0;
		
		double alternativeFitness = 0.0;
		if(ALTERNATIVE_FITNESS_CALCULATION_MODE.equals("min"))
		...
		for(Integer overwritingDefId : overwritingDefIds) {	
			...
			if(overwritingFitness <= 0.0 || overwritingFitness > SINGLE_ALTERNATIVE_FITNESS_RANGE)
				throw new IllegalStateException("expected this definition to be between >0 and SINLE_ALTERNATIVE_FITNESS_RANGE");
			...
			else if(ALTERNATIVE_FITNESS_CALCULATION_MODE.equals("max")) 
		...
		else if(ALTERNATIVE_FITNESS_CALCULATION_MODE.equals("avg"))
			alternativeFitness = alternativeFitness / overwritingDefIds.size();
		if(alternativeFitness<=0 || alternativeFitness>1)
			throw new IllegalStateException("calculated alternative fitness out of bounds");
//		System.out.println("calculated alternative fitness: "+alternativeFitness);		
	 ...
	 *  the above described fitness is linearly stretched to be in the interval (0,SINGLE_ALTERNATIVE_FITNESS_RANGE]
	 */
...
//				preFitnessDebugInfo(result,false);
	...
	public static double calculateUseFitnessForCompleteTrace(Use targetUse, 
		...
		if(hasEntriesForId(result.trace.passedUses.get(targetUse.getDUVariableName()),targetUse.getUseId()))
			return 0.0;
		// return calculated fitness
		return targetFitness.getFitness(individual, result);
	}
	 ...
	 * 
	 * To avoid making unnecessary calculations only calculate the fitness
	 * if the definition coming before goalDefinitionPos was not the goalDefinition.
	 * If it was, 1 is returned.  
	 * 
	 */
	public static double calculateUseFitnessForDefinitionPos(Definition targetDefinition,
		...
		int previousDefId = DefUseExecutionTraceAnalyzer.getPreviousDefinitionId(targetDefinition.getDUVariableName(),targetTrace,goalDefinitionPos,objectId);
		if(previousDefId == targetDefinition.getDefId())
			return 1;
		
		int overwritingDefPos = DefUseExecutionTraceAnalyzer.getNextOverwritingDefinitionPosition(targetDefinition, targetTrace,goalDefinitionPos,objectId);
		double fitness = calculateFitnessForDURange(individual, result, targetTrace, objectId, 
				targetUseBranchTestFitness, targetUse, true, goalDefinitionPos, overwritingDefPos);
	 ...
	 *   
	 */
	public static double calculateFitnessForDURange(TestChromosome individual, ExecutionResult result, 
		...
		if(fitness == 0.0 && wantToCoverTargetDU) {
			...
			DefUseExecutionTraceAnalyzer.printFinishCalls(cutTrace);
			throw new IllegalStateException("use cant have fitness 0 in this cut trace: "+cutTrace.toDefUseTraceInformation(targetDU.getDUVariableName(), objectId));
		}
		return fitness;		
	}
	 ...
	 */
	private static Set<Integer> getObjectPool(DefUseCoverageTestFitness goal, ExecutionTrace trace) {
	...
	public static boolean traceCoversGoal(DefUseCoverageTestFitness goal, Chromosome individual, ExecutionTrace trace) {
		...
		for(Integer objectID : objectPool) {
			...
			for(Integer usePos : usePositions) {
				...
				if (DefUseExecutionTraceAnalyzer.getActiveDefinitionIdAt(goalVariable, trace, usePos, objectID) == goalDefinition.getDefId())
					return true;
			}
		}
	...
	/**
	 * Creates a BranchCoverageTestFitness for the branch the given
	 * CFGVertex is control dependent on but considering the given targetExpressionValue
	 * as the branchExpressionValue
	 */
	public static BranchCoverageTestFitness getBranchTestFitness(CFGVertex v, boolean targetExpressionValue) {
