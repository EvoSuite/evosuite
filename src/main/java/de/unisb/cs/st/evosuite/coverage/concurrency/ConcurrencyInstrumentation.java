...
public class ConcurrencyInstrumentation implements MethodInstrumentation {
	...
	public void analyze(MethodNode mn, String className, String methodName, int access) {
		...
		while (instructions.hasNext()) {
			...
			for (BytecodeInstruction v : completeCFG.vertexSet()) {
				...
				// }
				// #TODO steenbuck the true should be some command line option
				// to activate the concurrency stuff
				if (true && instruction.equals(v.getASMNode()) && v.isFieldUse()
						&& (instruction instanceof FieldInsnNode) &&
						// #FIXME steenbuck we should also instrument fields,
						// which access primitive types.
						// #FIXME steenbuck apparently some objects (like
						// Boolean, Integer) are not working with this, likely a
						// different Signature (disappears when all
						// getfield/getstatic points are instrumented)
						((FieldInsnNode) instruction).desc.startsWith("L")) { // we
																				// only
																				// want
																				// objects,
																				// as
					...
					/*
					 * if (v.isDefinition()) DefUsePool.addDefinition(v);
					 * 
					 * // keeping track of uses if (v.isUse())
					 * DefUsePool.addUse(v);
					 */
	...
	private InsnList getConcurrencyInstrumentation(BytecodeInstruction v, int currentBranch) {
		...
		case Opcodes.GETSTATIC:
			// System.out.println("as seen in instrument:" + v.node.getClass() +
			// " branchID: " + currentBranch + " line " + v.line_no);
			...
			instrumentation.add(new MethodInsnNode(Opcodes.INVOKESTATIC, LockRuntime.RUNTIME_CLASS,
					LockRuntime.RUNTIME_SCHEDULER_CALL_METHOD, "(Ljava/lang/Object;I)V"));
			break;
		default:
			// Should never be reached. As the method calling this method checks
			// for the opcodes
