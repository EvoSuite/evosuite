...
public class ConcurrentTestRunnable implements InterfaceTestRunnable {
	...
	private static boolean print_to_system = false;// Properties.getPropertyOrDefault("print_to_system",
													// true);
	...
	// this is used to guarantee that each schedule is only logged once (there
	// might be a problem with the last schedule, as it is executed once more
	 ...
	 * 
	 * we first call the setup stuff to get us an object of the type under test,
	 * and then start two threads giving them the generated object
	 */
	@Override
	public ExecutionResult call() {
		...
		assert (m.keySet().size() == 0);// TODO steenbuck for testing, in
										// reality exceptions might be thrown.
		...
		for (int i = 0; i < ConcurrencyCoverageFactory.THREAD_COUNT; i++) {
			...
			TestRunnable testRunner = new TestRunnable(testToExecute, new ConcurrentScope(s.get(objectToTest)),
					observers);
		...
		try {
			// #TODO do we need to combine the execution results? We will if the
			// two threads run on different code
		...
		/*
		 * try { //System.out.println(r++); TestRunnable t2 = new
		 * TestRunnable(test.clone(), new ConcurrentScope(s.get(objectToTest)),
		 * observers); FutureTask<ExecutionResult> f2 = new
		 * FutureTask<ExecutionResult>(t2); Thread t= new Thread(f2);
		 * UncaughtExceptionHandler han = new UncaughtExceptionHandler() {
		 * 
		 * @Override public void uncaughtException(Thread t, Throwable e) {
		 * System.out.println("uncaught exception" + t); e.printStackTrace(); }
		 ...
		 * //f2.run();
		 * 
		 * ExecutionResult result = f2.get(); //ConcurrentTestCaseExecutor
		 * exe=ConcurrentTestCaseExecutor.getInstance(); //ExecutionResult
		 * result = exe.execute(test.clone(), new
		 * ConcurrentScope(s.get(objectToTest))); if(false)throw new
		 * InterruptedException(); if(false)throw new ExecutionException(null);
		 * //System.exit(1); //ExecutionResult result=null; //System.exit(1);
		 * assert(result!=null); return result; } catch (InterruptedException e)
		 * { // TODO Auto-generated catch block e.printStackTrace();
		 * System.exit(1); return null; } catch (ExecutionException e) { // TODO
		 * Auto-generated catch block e.printStackTrace(); System.exit(1);
		 * return null; }catch (Throwable e){ e.printStackTrace();
	...
	private ConcurrentTestCase addThreadEndCode(ConcurrentTestCase test) {
		...
		test.addStatement(new MethodStatement(test, threadEnd, null, Void.class, new ArrayList<VariableReference>()),
				false);
		return test;
	}
	...
	private ConcurrentTestCase addThreadRegistrationStatements(ConcurrentTestCase test) {
		try {
			...
			StatementInterface idst = new PrimitiveStatement<Integer>(test, Integer.class,
			...
			logger.warn("Tried to get method " + LockRuntime.RUNTIME_REGISTER_THREAD_METHOD
	...
	private ExecutionResult execute(BasicTestCase localTest, Scope localScope,
		...
		// TestCluster.getConstructors(Triangle.class).size());
		// ;
		// System.exit(-1);
	...
	private VariableReference getInitialTest(BasicTestCase tc, Class<?> clazz) {
		...
		for (Constructor<?> c : consts) {
			try {
				VariableReference v = tf.addConstructor(tc, c, 0, 0); // not the
																		// nicest
																		// way
																		// to
																		// handle
				...
				// #TODO steenbuck in the long run this shouldn't be an
				// exception but normal control flow
				System.exit(-1);
				return null;
			}
		}
