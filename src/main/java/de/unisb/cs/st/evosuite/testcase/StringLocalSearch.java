...
public class StringLocalSearch implements LocalSearch {
	...
	public void doSearch(TestChromosome test, int statement, LocalSearchObjective objective) {
		...
		// TODO: First apply 10 random mutations to determine if string
		// influences uncovered branch
		...
		if (affected) {
			...
			// Second, try to replace each of the characters with each of the 64
			// possible characters
	...
	private boolean addCharacters(LocalSearchObjective objective, TestChromosome test, PrimitiveStatement<String> p,
			int statement) {
		...
		while (add) {
			...
			for (char replacement = 0; replacement < 128; replacement++) {
				...
				// logger.info(" " + oldValue + "/" + oldValue.length() + " -> "
				// + newString
				// + "/" + newString.length());
	...
	private boolean removeCharacters(LocalSearchObjective objective, TestChromosome test, PrimitiveStatement<String> p,
			int statement) {
	...
	private boolean replaceCharacters(LocalSearchObjective objective, TestChromosome test,
