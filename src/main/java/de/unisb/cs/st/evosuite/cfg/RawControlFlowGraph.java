...
public class RawControlFlowGraph extends ControlFlowGraph<BytecodeInstruction> {
	...
	public int getControlDistance(List<Integer> path, int vertex) {
		 ...
		 * logger.info("Graph vertex: "+v.id); for(ControlFlowEdge edge :
		 * graph.outgoingEdgesOf(v)) {
		...
		for (Integer i : path) {
			...
			// FIXME: Problem: i is not a bytecode id, but a branch ID (or a
			// line id). What can we do?
	...
	private Set<Use> getUsesForDef(Definition targetDef, BytecodeInstruction entry) {
		...
		for (ControlFlowEdge e : outgoingEdges) {
			...
			if (edgeTarget.getInstructionId() > entry.getInstructionId()) {
	...
	private boolean hasDefClearPathToMethodExit(DefUse targetDefUse, BytecodeInstruction currentVertex) {
		...
		for (ControlFlowEdge e : outgoingEdges) {
			...
			if ((edgeTarget.getInstructionId() > currentVertex.getInstructionId() // dont
																					// follow
																					// backedges
																					// (loops)
					)
	...
	/*
	 * private boolean isWhileBranch(int maxID) { BytecodeInstruction prevVertex
	 * = getInstruction(maxID - 1); Set<ControlFlowEdge> prevOut =
	 * graph.outgoingEdgesOf(prevVertex); if (prevOut.size() != 1) { return
	 * false; } ControlFlowEdge backEdge = null; for (ControlFlowEdge e :
	 * prevOut) backEdge = e; // only while-branches go back up return
	 * graph.getEdgeTarget(backEdge).getId() < maxID; }
	 */
	...
	// // if(isWhileBranch(maxID)) // accepts for-loops when they dont have a
	// return
	...
	// private void markNodes(int start, int end, BytecodeInstruction branch,
