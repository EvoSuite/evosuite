...
public class ConcurrencyTracer {
	...
	private static int computeLevenshteinDistance(List<SchedulingDecisionTuple> str1, List<SchedulingDecisionTuple> str2) {
		...
		for (int i = 1; i <= str1.size(); i++) {
			for (int j = 1; j <= str2.size(); j++) {
				...
				if (!str1.get(i - 1).equals(str2.get(j - 1))) {
					int cost = (str1.get(i - 1).equals(str2.get(j - 1))) ? 0 : 1;

					distance[i][j] = minimum(distance[i - 1][j] + 1, // delete
							distance[i][j - 1] + 1, // insert
							distance[i - 1][j - 1] + cost); // substitute
					if ((i > 1) && (j < str2.size()) && (i < str1.size()) && (j > 1)
							&& str1.get(i).equals(str2.get(j - 1)) && str1.get(i - 1).equals(str2.get(j))) {
						// System.out.println("transpose " + distance[i][j] +
	...
	private static int test(List<SchedulingDecisionTuple> goal, List<SchedulingDecisionTuple> run) {
		...
		for (SchedulingDecisionTuple t : run) {
			for (searchState s : states) {
				// assert(goal.size()>s.pos) : "goal size is only " +
				// goal.size() + " but we will request pos " + s.pos;
		...
		// ConcurrencySuitCoverage.printList(goal));
		// System.out.println("reality:" +
		// ConcurrencySuitCoverage.printList(run));
	...
	// #TODO global naming for scheduleID (the IDs written to the source code at
	// each schedule point). This is named requestID in other places
