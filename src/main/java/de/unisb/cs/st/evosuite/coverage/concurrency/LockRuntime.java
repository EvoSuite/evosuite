...
public class LockRuntime {
	...
	public static final Set<Integer> threadIDs = new HashSet<Integer>(); // #TODO
																			// we
																			// make
																			// the
																			// assumption,
	...
	private volatile static int fieldAccessID = 0; // A counter to generate a
													// unique ID for each
													// occurrence of a field
													// (class or object) access.
	...
	public static int getUniqueThreadID() {
		try {
			...
			logger.trace("A unique thread ID was requested from the lockruntime, by the thread "
					+ Thread.currentThread() + ". Which forwarded the request to the controller "
					+ controller.toString());
	...
	public static void scheduler(Object requested, int requestID) {
		try {
			...
			// if(true)return;
			// System.out.println(requested + " is requested by " +
			// controller.getThreadID(Thread.currentThread()) + " requesterID "
			// + " at " + requestID);
			assert (controller != null); // needs to be set somewhere
			assert (tracer != null); // maybe we later need to be able to run
										// without a tracer (a dummy could be
										// provided)
			...
			try {
				...
				synchronized (controller.threadWaitingPos.get(Thread.currentThread())) {
					...
					controller.awake(); // this is going to stale for the
										// current Thread as the controller
										// won't be able to get the
										// threadWaitingPos.get(Thread.currentThread())
										// monitor
			...
			// System.out.println(requested + " was granted to " +
			// controller.getThreadID(Thread.currentThread()) + " requesterID "
			// + " at " + requestID);
		} catch (Throwable e) {
			throw new EvosuiteError(e);
