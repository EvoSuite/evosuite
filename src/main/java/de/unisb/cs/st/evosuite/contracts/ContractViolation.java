...
public class ContractViolation {
	...
	public ContractViolation(Contract contract, TestCase test, StatementInterface statement, Throwable exception) {
	...
	public int hashCode() {
		...
		if (statement != null) {
			if (statement instanceof MethodStatement) {
				MethodStatement ms1 = (MethodStatement) statement;
				result = prime * result + ms1.getMethod().getDeclaringClass().hashCode() + ms1.getMethod().hashCode();
			} else if (statement instanceof ConstructorStatement) {
				ConstructorStatement ms1 = (ConstructorStatement) statement;
				result = prime * result + ms1.getConstructor().getDeclaringClass().hashCode()
