...
public class StringReplacementFunctions {
	...
	public static int equalsDistance(String first, String second) {
		if (first.equals(second)) {
			...
			/*
			 * int l1 = first.length(); int l2 = second.length(); int distance =
			 * Math.abs(l1 - l2); for(int i = 0; i<Math.min(l1, l2); i++) {
			 * if(first.charAt(i) != second.charAt(i)) distance++; } return
			 * distance;
			 */
		}
	}
	...
	public static int OED(String s1, String s2) {
		// The edit distance of an empty string and a given string is the length
		// of the given string
		int n = s1.length();
		int m = s2.length();
		if (n == 0) {
