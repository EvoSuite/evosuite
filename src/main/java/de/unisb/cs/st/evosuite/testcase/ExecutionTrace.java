...
public class ExecutionTrace {
	...
	public class MethodCall {
		...
		public MethodCall clone() {
			MethodCall copy = new MethodCall(className, methodName, methodId, callingObjectID);
	...
	private static void checkSaneCall(MethodCall call) {
		if (!((call.trueDistanceTrace.size() == call.falseDistanceTrace.size())
				&& (call.falseDistanceTrace.size() == call.defuseCounterTrace.size()) && (call.defuseCounterTrace
				.size() == call.branchTrace.size()))) {
	 ...
	 * removableCalls
	 */
	private static void removeFinishCalls(ExecutionTrace trace, ArrayList<Integer> removableCalls) {
	...
	private static void removeFromFinishCall(MethodCall call, ArrayList<Integer> removableIndices) {
		...
		for (int i = removableIndices.size() - 1; i >= 0; i--) {
			...
			if ((removedCounter == null) || (removedBranch == null) || (removedTrue == null) || (removedFalse == null)) {
	 ...
	 */
	public void definitionPassed(String className, String varName, String methodName, Object caller, int branchID,
			int defID) {
			...
			for (int i = 0; i < call.defuseCounterTrace.size(); i++) {
				...
				if ((currentDUCounter < duCounterStart) || (currentDUCounter > duCounterEnd)) {
					...
					if (targetExpressionValue) {
						// TODO as mentioned in
						// CFGVertex.branchExpressionValue-comment: flip it!
	...
	public int hashCode() {
		...
		result = prime * result + ((finished_calls == null) ? 0 : finished_calls.hashCode());
	...
	public void returnValue(String className, String methodName, int value) {
		...
		if (!return_data.get(className).get(methodName).containsKey(value)) {
			...
			return_data.get(className).get(methodName)
					.put(value, return_data.get(className).get(methodName).get(value) + 1);
		}
	}
	 ...
	 */
	public void usePassed(String className, String varName, String methodName, Object caller, int branchID, int useID) {
