...
public class UsageModel {
	...
	private final Map<String, Integer> models = new HashMap<String, Integer>();
	private final Map<String, Integer> constraints = new HashMap<String, Integer>();
	...
	private final Map<String, ClassUsage> usage_models = new HashMap<String, ClassUsage>();
	 ...
	 * @param method
	 *            - the method/constructor that needs the parameter
	 * @param parameter
	 ...
	 * @return
	 */
	public boolean hasGenerator(String className, ConcreteCall method, int parameter,
	        ConcreteCall generator) {
		if (!usage_models.containsKey(className))
			return false;
	 ...
	 * 
	 * @param className
	 *            - name of the class that should be constructed
	 * @return
	 */
	 ...
	 * @param sourceMethod
	 */
	private void updateParameterModel(String className, String method, int parameter,
		...
		if (ensureClassModel(className)) {
			...
			logger.debug("Adding parameter usage: " + target_call + " parameter "
			        + parameter + ": " + source_call);
			// TODO: Need to store concrete class sourceClass
			usage_models.get(className).addParameterUsage(target_call, parameter,
			                                              source_call); // TODO: This could be more efficient
		}
	}
	...
	private void updateUsageModel(String className, String method1, String method2) {
		if (ensureClassModel(className)) {
			...
			logger.debug("Adding usage transition for class " + className + ": " + call1
			        + " -> " + call2);
	...
	private void updateModel(String className, Transition left, Transition right) {
		...
		if (!className.startsWith(Properties.PROJECT_PREFIX)) {
			logger.debug("Ignoring pair outside project prefix: "
			        + left.getShortEventString() + " -> " + right.getShortEventString());
			return;
		}
		if (right instanceof InvokeMethodTransition) {
			...
			//	return;
			//}
			logger.debug("Current pair: " + left.getShortEventString() + " -> "
			        + right.getShortEventString());
			...
			if (left instanceof InvokeMethodTransition) {
				...
				if (param1 == 0 && param2 == 0) {
					...
					updateUsageModel(className, t1.getMethodCall().getMethodName(),
					                 t2.getMethodCall().getMethodName());

					// If LHS is a constructor this is a generator
					if (t1.getMethodCall().getMethodName().contains("<init>")) {
						updateGenerator(className, t1.getMethodCall().getTypeName(),
						                t1.getMethodCall().getMethodName());
					}
				}
				...
				if (param1 == 0) { //&& !t1.getMethodCall().getTypeName().equals("java.lang.Object")) {
					...
					updateParameterModel(className, t2.getMethodCall().getMethodName(),
					                     param2, t1.getMethodCall().getTypeName(),
					                     t1.getMethodCall().getMethodName());
			...
			logger.trace("Unknown transition B: " + left.getShortEventString() + " -> "
			        + right.getShortEventString());
		}
	}
				...
				public void visit(int id, Model model, ModelData modelData) {
					...
					if (!className.startsWith(Properties.PROJECT_PREFIX)) {
						if (!className.startsWith("java")
							...
							logger.debug("Ignoring model for external class: "
							        + className);
					} else {
						if (!models.containsKey(className)) {
							...
							constraints.put(className, models.get(className)
							        + model.getAllTransitions().size());
						}

						logger.debug("Current model: " + modelData.getModelName()
						        + " generated in " + modelData.getClassName() + "."
						        + modelData.getMethodName());
						logger.debug("   Number of transitions: "
						        + model.getAllTransitions().size());
						if (model.getAllTransitions().size() == 1) {
							for (Transition t : model.getAllTransitions()) {
								...
								if (t instanceof InvokeMethodTransition) {
									...
									if (mt.getMethodCall().getTypeName().equals(className)
									        && mt.getMethodCall().getMethodName().contains("<init>")
									        && mt.getParameterIndices().contains(0)) {
										updateGenerator(className,
										                mt.getMethodCall().getTypeName(),
										                mt.getMethodCall().getMethodName());
									}
									...
									updateGenerator(className,
									                rt.getMethodCall().getTypeName(),
									                rt.getMethodCall().getMethodName());
						...
						Set<EventPair> sca_model = SCAAbstractor.getSCAAbstraction(model,
						                                                           true);
	...
	public static ConcreteCall getMethod(String className, String methodName) {
		...
		if (o == null) {
			logger.debug("Could not find method: " + methodName + " in class "
			        + className);
			return null;
		}
