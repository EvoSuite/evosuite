...
public class GenericClass {
	...
	public static boolean isAssignable(Type lhsType, Type rhsType) {
		...
		if ((lhsType instanceof Class<?>) && (rhsType instanceof Class<?>)) {
			// if(ClassUtils.isAssignable((Class<?>) rhsType, (Class<?>)
			// lhsType)) {
		...
		// ParameterizedType) {
		// return isAssignable((ParameterizedType) lhsType, (ParameterizedType)
		// rhsType);
		// }
	...
	private static boolean isAssignable(WildcardType lhsType, Type rhsType) {
		/*
		 * Type[] upperBounds = lhsType.getUpperBounds(); Type[] lowerBounds =
		 * lhsType.getLowerBounds(); for (int size = upperBounds.length, i = 0;
		 * i < size; ++i) { if (!isAssignable(upperBounds[i], rhsType)) { return
		 * false; } } for (int size = lowerBounds.length, i = 0; i < size; ++i)
		 * { if (!isAssignable(rhsType, lowerBounds[i])) { return false; } }
		 */
		return true;
	}
	...
	/*
	 * public boolean equals(GenericClass other) { return
