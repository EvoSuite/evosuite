...
public class AmbigueUIState extends AbstractUIState implements GraphVizDrawable {
	...
	private Map<DescriptorBoundUIAction<?>, UIState> ownTransitions = new HashMap<DescriptorBoundUIAction<?>, UIState>();
	...
	private void addPossibleStateInternal(AbstractUIState state, int count) {
		...
		if (!state.isKnown()) {
			throw new IllegalArgumentException("Cannot add an unknown state as possible state of an AmbigueUIState");
		}
		else {
			this.possibleStates.put(state, count + (!this.possibleStates.containsKey(state) ? 0 : this.possibleStates.get(state)));
		}
	}
	...
	public void addTransition(DescriptorBoundUIAction<?> action, UIState toState) {
		throw new UnsupportedOperationException("Why are you trying to add a transition to an AmbigueUIState?");
	...
	public Map<DescriptorBoundUIAction<?>, AbstractUIState> getTransitions() {
		Map<DescriptorBoundUIAction<?>, AbstractUIState> result = new HashMap<DescriptorBoundUIAction<?>, AbstractUIState>(this.ownTransitions);
		...
		for (AbstractUIState possibleState : possibleStates.keySet()) {
			...
			for (Map.Entry<DescriptorBoundUIAction<?>, ? extends AbstractUIState> entry : transitions.entrySet()) {
				...
				if (result.containsKey(key)) {
					...
					result.put(key, new AmbigueUIState(Arrays.<AbstractUIState>asList(newState, oldState)));
				}
			}
		}
	...
	public String shortString() {
		...
		for (Map.Entry<AbstractUIState, Integer> entry : possibleStates.entrySet()) {
			...
			result.append(String.format("%s=%s", entry.getKey().shortString(), entry.getValue()));
			isFirst = false;
		}
