...
public class TestSuiteGenerator {
	...
	public List<TestCase> generateIndividualTests(GeneticAlgorithm ga) {
		...
		if (Properties.PREORDER_GOALS_BY_DIFFICULTY) {
			...
			System.out.println("* Time taken for difficulty computation: " + DefUseCoverageTestFitness.difficulty_time
					+ "ms");
		} else {
		...
		while ((current_budget < total_budget) && (covered_goals < total_goals) && !global_time.isFinished()) {
			...
			for (TestFitnessFunction fitness_function : goals) {
				...
				if (ga.getBestIndividual().getFitness() == 0.0) {
					...
					logger.info("Found no solution at " + MaxStatementsStoppingCondition.getNumExecutedStatements());
				}
		...
		System.out.println("* Search finished after " + (end_time - start_time) + "s, best individual has fitness "
				+ suite.getFitness());
	...
	public List<TestCase> generateWholeSuite(GeneticAlgorithm ga) {
		...
		long end_time = System.currentTimeMillis() / 1000;
		System.out.println("* Search finished after " + (end_time - start_time) + "s and " + ga.getAge()
				+ " generations, best individual has fitness " + best.getFitness());
	...
	public GeneticAlgorithm setup() {
		...
		if (Properties.DYNAMIC_LIMIT) {
			...
			Properties.GENERATIONS = Properties.GENERATIONS
					* (BranchPool.getBranchlessMethods().size() + BranchPool.getBranchCounter() * 2); // TODO
																										// question:
																										// is
																										// branchMap.size()
																										// really
	 ...
	 */
	private Set<Integer> getAdditionallyCoveredGoals(List<TestFitnessFunction> goals, Set<Integer> covered,
			TestChromosome best) {
