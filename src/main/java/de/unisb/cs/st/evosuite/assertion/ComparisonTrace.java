...
public class ComparisonTrace extends OutputTrace {
	...
	public int getAssertions(TestCase test, OutputTrace other_trace) {
		...
		for (int line = 0; line < test.size(); line++) {
			...
			// Only add equals calls if we have no compare calls (else we'll get
			// both)
			if (have_assertion) {
				continue;
			}
	...
	public int numDiffer(OutputTrace other_trace) {
		...
		for (Entry<Integer, Map<VariableReference, Integer>> entry : compare_map.entrySet()) {
			if (other.compare_map.containsKey(entry.getKey())) {
				// if(entry.getValue() == null &&
				// other.compare_map.get(entry.getKey()) != null) {
				// num++;
				// }
				// else
				if (entry.getValue() != null) {
					for (Entry<VariableReference, Integer> centry : entry.getValue().entrySet()) {
						if (other.compare_map.get(entry.getKey()).containsKey(centry.getKey())
								&& !centry.getValue()
										.equals(other.compare_map.get(entry.getKey()).get(centry.getKey()))) {
							logger.debug("Retval of " + entry.getKey() + " comparing with " + centry.getKey());
							logger.debug("CompareTo: " + centry.getValue() + "/"
		...
		for (Entry<Integer, Map<VariableReference, Boolean>> entry : equals_map.entrySet()) {
			if (other.equals_map.containsKey(entry.getKey())) {
				// if(entry.getValue() == null &&
				// other.equals_map.get(entry.getKey()) != null) {
				// num++;
				// }
				// else
				if (entry.getValue() != null) {
					for (Entry<VariableReference, Boolean> centry : entry.getValue().entrySet()) {
						if (other.equals_map.get(entry.getKey()).containsKey(centry.getKey())
							...
							logger.debug("Equals: " + centry.getValue() + "/"
		...
		/*
		 * for(Entry<Integer, Map<VariableReference, Integer > > entry :
		 * other.compare_map.entrySet()) {
		 * if(!compare_map.containsKey(entry.getKey())) num++; }
		 * for(Entry<Integer, Map<VariableReference, Boolean > > entry :
		 * other.equals_map.entrySet()) {
		 * if(!compare_map.containsKey(entry.getKey())) num++; }
		 */
