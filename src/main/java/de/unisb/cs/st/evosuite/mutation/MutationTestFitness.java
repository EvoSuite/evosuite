...
public class MutationTestFitness extends TestFitnessFunction {
	...
	private Mutation target_mutation;
	
	private MutationGoal mutation_goal;
...
//			num += 0.5;
	...
	public double getFitness(TestChromosome individual, ExecutionResult result) {
		...
		/*
		ExecutionResult check_result = runTest(individual.test);
		int n = getNumAssertions(result, check_result);
		if(n > 0) {
			logger.info("Found change: ");
			logger.info(individual.test.toCode());
			result = check_result;
		}
		*/
		/*
		logger.info("Execution check 1");
		ExecutionResult mutant_result = runTest(individual.test, target_mutation);
		getNumAssertions(result, mutant_result);
		
		logger.info("Execution check 2");
		result = runTest(individual.test);
		getNumAssertions(result, mutant_result);

		logger.info("Execution check 3");
		mutant_result = runTest(individual.test, target_mutation);
		getNumAssertions(result, mutant_result);
*/
		// First, get the distance. TODO: What about branch distance?
		ControlFlowDistance cfg_distance = mutation_goal.getDistance(result);

		if(cfg_distance.approach > 1 || (cfg_distance.approach == 1 && cfg_distance.branch > 0.0)) {
			logger.debug("Distance is "+cfg_distance.approach+"/" + cfg_distance.branch);
			if(cfg_distance.approach == 1)
				logger.debug(individual.test.toCode());
//			return 1.0 + cfg_distance.approach + normalize(cfg_distance.branch);
			/*
			ExecutionResult mutant_result = runTest(individual.test, target_mutation);
			...
			*/
			return cfg_distance.approach + normalize(cfg_distance.branch); // 1 = distance for mutation activation
/*		} else if(cfg_distance.approach == 1) {
			logger.info("Distance is "+cfg_distance.approach+"/" + cfg_distance.branch);
			ExecutionResult mutant_result = runTest(individual.test, target_mutation);
			if(!mutant_result.touched) {
				logger.warn("Mutant not touched!");
			} else {
		...
		*/
		...
		if(!mutant_result.touched.contains(target_mutation.getId())) {
			logger.warn("Distance is 1 but mutant "+target_mutation.getId()+" not executed: "+target_mutation.getClassName()+"."+target_mutation.getMethodName());
...
//		if(!hasAssertions(result, mutant_result)) 
		if(getNumAssertions(result, mutant_result) == 0) {
			...
			/*
			individual.test.removeAssertions();
			AssertionGenerator gen = new AssertionGenerator();
			gen.addAssertions(individual.test, target_mutation);
			logger.info(individual.test.toCode());
	...
	public String toString() {
		return "Mutation goal: "+target_mutation.getId()+": "+target_mutation.getClassName()+"."+target_mutation.getMethodName()+":"+target_mutation.getLineNumber()+" - "+target_mutation.getMutationType();
	}
}
