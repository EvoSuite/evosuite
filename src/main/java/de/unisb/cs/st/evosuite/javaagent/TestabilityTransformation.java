 ...
 * 
 */
// TODO: If we transform a method and there already is a method with the
// transformed descriptor, we need to change the method name as well!
public class TestabilityTransformation {
	...
	public static String getOriginalNameDesc(String className, String methodName, String desc) {
		...
		if (mapping.originalDesc.containsKey(key)) {
			...
			return mapping.getOriginalName(className, methodName, desc) + mapping.originalDesc.get(key);
		} else {
			return methodName + desc;
		}
	}

	public static String getTransformedDesc(String className, String methodName, String desc) {
		return mapping.getMethodDesc(className, methodName, desc);
	}

	public static String getTransformedName(String className, String methodName, String desc) {
	...
	public ClassNode transform() {
		...
		// cn.interfaces.set(i,
		// n.replaceAll(Matcher.quoteReplacement("java/util"),
		// "java2/util2"));//
		//
		// }
		// cn.superName =
		// cn.superName.replaceAll(Matcher.quoteReplacement("java/util"),
		// "java2/util2");
	...
	private void insertDoubleComparison(AbstractInsnNode position, InsnList list) {
		...
		MethodInsnNode get = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
				"fromDouble", Type.getMethodDescriptor(Type.INT_TYPE, new Type[] { Type.DOUBLE_TYPE }));
	...
	private void insertFloatComparison(AbstractInsnNode position, InsnList list) {
		...
		MethodInsnNode get = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
				"fromFloat", Type.getMethodDescriptor(Type.INT_TYPE, new Type[] { Type.FLOAT_TYPE }));
		list.insert(position, get);
		list.remove(position);
	}
	...
	private void insertGet(AbstractInsnNode position, InsnList list) {
		MethodInsnNode get = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
				"getDistance", Type.getMethodDescriptor(Type.INT_TYPE, new Type[] { Type.INT_TYPE }));
		list.insert(position, get);
		// list.remove(position);
	}
	...
	private void insertLongComparison(AbstractInsnNode position, InsnList list) {
		...
		MethodInsnNode get = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
				"fromLong", Type.getMethodDescriptor(Type.INT_TYPE, new Type[] { Type.LONG_TYPE }));
		list.insert(position, get);
		list.remove(position);
	}
	...
	private void insertPush(int opcode, AbstractInsnNode position, InsnList list) {
		...
		MethodInsnNode push = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
				"pushPredicate", Type.getMethodDescriptor(Type.VOID_TYPE, new Type[] { Type.INT_TYPE }));
	...
	private void insertPushEquals(int opcode, AbstractInsnNode position, InsnList list) {
		MethodInsnNode equalCheck = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
				"isEqual", Type.getMethodDescriptor(Type.INT_TYPE,
	...
	private void insertPushNull(int opcode, AbstractInsnNode position, InsnList list) {
		MethodInsnNode nullCheck = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
				"isNull", Type.getMethodDescriptor(Type.INT_TYPE, new Type[] { Type.getType(Object.class),
						Type.INT_TYPE }));
		list.insertBefore(position, new InsnNode(Opcodes.DUP));
		list.insertBefore(position, new LdcInsnNode(opcode));
	...
	private boolean isBooleanAssignment(AbstractInsnNode position, MethodNode mn) {
		...
		while (!done) {
			if ((node.getOpcode() == Opcodes.PUTFIELD) || (node.getOpcode() == Opcodes.PUTSTATIC)) {
				...
				// if it is a boolean parameter of a converted method, then it
				// needs to be converted
				// Problem: How do we know which parameter it represents?

				logger.warn("Cannot handle method insn?"); // TODO: Just got to
															// check last
															// parameter?
				return false;
	...
	private void processFields() {
		...
		for (FieldNode fn : fieldNodes) {
			...
			// fn.desc =
			// fn.desc.replaceAll(Matcher.quoteReplacement("java/util"),
			// "java2/util2");
	...
	private void processMethods() {
		...
		for (MethodNode mn : methodNodes) {
			// If this method was defined somewhere outside the test package, do
			// not transform signature
			String desc = mn.desc;
			mn.desc = mapping.getMethodDesc(cn.name, mn.name, mn.desc);
			// mn.desc =
			// mn.desc.replaceAll(Matcher.quoteReplacement("java/util"),
			// "java2/util2");
			//
		...
		System.out.println("Flag definitions found for class " + cn.name + ": " + (flagDefs.size() - defs));
				...
				if (node.getOpcode() == Opcodes.BALOAD) {
					...
					if (frames == null) {
						...
						if (current.getStack(size - 2) == BooleanArrayInterpreter.INT_ARRAY) {
							...
							logger.info("XXX Stack has size " + size + ", expecting array at " + (size - 2));
					...
					if (frames == null) {
						...
						if (current.getStack(size - 3) == BooleanArrayInterpreter.INT_ARRAY) {
							...
							logger.info("Stack has size " + size + ", expecting array at " + (size - 3));
					...
					if (min.owner.equals("[Z")) {
						if (frames == null) {
							mn.instructions.insertBefore(node, new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "[I",
									min.name, min.desc));
							mn.instructions.remove(node);
							changed = true;
					...
					if (tin.desc.equals("[Z")) {
						...
						if (frames == null) {
							mn.instructions.insertBefore(node, new TypeInsnNode(Opcodes.CHECKCAST, "[I"));
							mn.instructions.remove(node);
							changed = true;
				...
				if (isBooleanAssignment(node, mn)) {
					if (node.getOpcode() == Opcodes.IOR) {
						MethodInsnNode push = new MethodInsnNode(Opcodes.INVOKESTATIC,
								Type.getInternalName(BooleanHelper.class), "IOR", Type.getMethodDescriptor(
										Type.INT_TYPE, new Type[] { Type.INT_TYPE, Type.INT_TYPE }));
						mn.instructions.insertBefore(node, push);
						mn.instructions.remove(node);
					} else if (node.getOpcode() == Opcodes.IAND) {
						MethodInsnNode push = new MethodInsnNode(Opcodes.INVOKESTATIC,
								Type.getInternalName(BooleanHelper.class), "IAND", Type.getMethodDescriptor(
										Type.INT_TYPE, new Type[] { Type.INT_TYPE, Type.INT_TYPE }));
						mn.instructions.insertBefore(node, push);
						mn.instructions.remove(node);
						...
						MethodInsnNode push = new MethodInsnNode(Opcodes.INVOKESTATIC,
								Type.getInternalName(BooleanHelper.class), "IXOR", Type.getMethodDescriptor(
										Type.INT_TYPE, new Type[] { Type.INT_TYPE, Type.INT_TYPE }));
						mn.instructions.insertBefore(node, push);
						mn.instructions.remove(node);
		...
		while (node != mn.instructions.getLast()) {
			...
			if (node instanceof InsnNode) {
				...
				if ((in.getOpcode() == Opcodes.ICONST_0) && isBooleanAssignment(node, mn)) {
					...
					flagDefs.add(node);
				} else if ((in.getOpcode() == Opcodes.ICONST_1) && isBooleanAssignment(node, mn)) {
					insertGet(node, mn.instructions);
					flagDefs.add(node);
				}
				...
				if (isBooleanVariable(vn1.var, mn) && (node.getNext() instanceof VarInsnNode)) {
				...
				if (mapping.isTransformedOrBooleanField(fn1.owner, fn1.name, fn1.desc)
					...
					if (fn1.owner.equals(fn2.owner) && fn1.name.equals(fn2.name) && fn1.desc.equals(fn2.desc)) {
						if ((fn1.getOpcode() == Opcodes.GETFIELD) && (fn2.getOpcode() == Opcodes.PUTFIELD)) {
							insertGet(node, mn.instructions);
							flagDefs.add(node);
						} else if ((fn1.getOpcode() == Opcodes.GETSTATIC) && (fn2.getOpcode() == Opcodes.PUTSTATIC)) {
							insertGet(node, mn.instructions);
							flagDefs.add(node);
						}
					}
	...
	private void transformBooleanPredicates(MethodNode mn) {
		...
		while (node != mn.instructions.getLast()) {
			if (node instanceof JumpInsnNode) {
				...
				switch (jn.getOpcode()) {
				// TODO: Only insert this push if we are comparing ints? -> Not
				// for flag usage!
				case Opcodes.IFEQ:
				case Opcodes.IFNE:
				case Opcodes.IFLT:
			...
			while (iterator.hasNext()) {
				...
				if (node.getOpcode() == Opcodes.IRETURN) {
					MethodInsnNode n = new MethodInsnNode(Opcodes.INVOKESTATIC,
							Type.getInternalName(BooleanHelper.class), "intToBoolean", Type.getMethodDescriptor(
									Type.BOOLEAN_TYPE, new Type[] { Type.INT_TYPE }));
	...
	private void transformFieldInsn(FieldInsnNode node, MethodNode mn) {
		...
		if (Type.getType(node.desc).equals(Type.BOOLEAN_TYPE)) {
			if ((node.getOpcode() == Opcodes.PUTFIELD) || (node.getOpcode() == Opcodes.PUTSTATIC)) {
				...
				MethodInsnNode n = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
						"booleanToInt", Type.getMethodDescriptor(Type.INT_TYPE, new Type[] { Type.BOOLEAN_TYPE }));
			...
			if (jump != null) {
				if (node instanceof MethodInsnNode) {
					...
					if (me.name.equals("instanceOf")
							|| mapping.isTransformedOrBooleanReturnMethod(me.owner, me.name, me.desc)) {
						...
						logger.info("This does not look like a transformable if: " + me.owner + "." + me.name);
					}
				} else if (node instanceof FieldInsnNode) {
					logger.info("Found IFELSE");
					FieldInsnNode fn = (FieldInsnNode) node;
					if (((fn.getOpcode() == Opcodes.GETFIELD) || (fn.getOpcode() == Opcodes.GETSTATIC))
							&& mapping.isTransformedOrBooleanField(fn.owner, fn.name, fn.desc)) {
						transformBooleanIf(jump, mn);
					}
				} else if (node instanceof VarInsnNode) {
					logger.info("Found IFELSE");
	...
	private void transformImplicitElse(MethodNode mn) {
		...
		// TODO: we might also store this in a field (PUTSTATIC /
		// ALOAD+PUTFIELD)
		...
		while ((node != null) && (node != mn.instructions.getLast())) {
			...
			logger.info("Match starting with opcode " + node.getOpcode() + " at position "
					+ mn.instructions.indexOf(node));
			logger.info("Regex starting with opcode " + NodeRegularExpression.STOREFLAG.pattern[0][0]);
			...
			while ((m != target) && (m != mn.instructions.getLast())) {
				if (m instanceof VarInsnNode) {
					...
					if ((lvn != null) && (mn.instructions.indexOf(lvn.end) > mn.instructions.indexOf(target))
							&& lvn.desc.equals("Z")) {
						num = vn.var;
					}
				}
			...
			if (m != mn.instructions.getLast()) {
				node = NodeRegularExpression.STOREFLAG.getNextMatch(m.getNext(), mn.instructions);
			} else {
	...
	private void transformImplicitElseField(MethodNode mn) {
		...
		while ((node != null) && (node != mn.instructions.getLast())) {
			...
			logger.info("Regex starting with opcode " + NodeRegularExpression.STOREFLAG3.pattern[0][0]);
			...
			if (target.getPrevious() instanceof FieldInsnNode) {
				...
				mn.instructions.insertBefore(target, new JumpInsnNode(Opcodes.GOTO, l));
				mn.instructions.insert(target, l);
				mn.instructions.insert(target, new FieldInsnNode(Opcodes.PUTFIELD, fn.owner, fn.name, fn.desc));
				mn.instructions.insert(target, new FieldInsnNode(Opcodes.GETFIELD, fn.owner, fn.name, fn.desc));
				mn.instructions.insert(target, new VarInsnNode(vn.getOpcode(), vn.var));
				mn.instructions.insert(target, new VarInsnNode(vn.getOpcode(), vn.var));
			}
			// Find label node
			// After label, add new ILOAD_2 ISTORE_2
			node = NodeRegularExpression.STOREFLAG3.getNextMatch(node.getNext(), mn.instructions);
		}
	}
	...
	private void transformImplicitElseStaticField(MethodNode mn) {
		...
		while ((node != null) && (node != mn.instructions.getLast())) {
			...
			logger.info("Found potential un-expanded else: ");
			logger.info("Match starting with opcode " + node.getOpcode());
			logger.info("Regex starting with opcode " + NodeRegularExpression.STOREFLAG2.pattern[0][0]);
			...
			if (num >= 0) {
				...
				mn.instructions.insertBefore(target, new JumpInsnNode(Opcodes.GOTO, l));
				mn.instructions.insert(target, l);
				mn.instructions.insert(target, new FieldInsnNode(Opcodes.PUTSTATIC, fn.owner, fn.name, fn.desc));
				mn.instructions.insert(target, new FieldInsnNode(Opcodes.GETSTATIC, fn.owner, fn.name, fn.desc));
			}

			// Find label node
			// After label, add new ILOAD_2 ISTORE_2
			node = NodeRegularExpression.STOREFLAG2.getNextMatch(m.getNext(), mn.instructions);
		}
	}
	...
	private int transformMethod(MethodNode mn) {
		...
		MethodInsnNode reset = new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
				"clearPredicates", Type.getMethodDescriptor(Type.VOID_TYPE, new Type[] {}));
		...
		// Convert information about local variables (only needed for debugging
		// really)
	...
	private void transformMethodInsn(MethodInsnNode node, MethodNode mn) {
		...
		if (mapping.hasBooleanParameters(node.desc)) {
					...
					for (int i = 0; i < numOfPushs; i++) {
						...
						if ((types[types.length - 1 - i] == Type.BOOLEAN_TYPE)
							...
							if (types[types.length - 1 - i] == Type.BOOLEAN_TYPE) {
								...
								booleanHelperPushParameter = new MethodInsnNode(Opcodes.INVOKESTATIC,
										Type.getInternalName(BooleanHelper.class), "pushParameter",
										Type.getMethodDescriptor(Type.VOID_TYPE, new Type[] { types[types.length - 1
												- i] }));
							}
						} else {
							booleanHelperPushParameter = new MethodInsnNode(Opcodes.INVOKESTATIC,
					...
					for (int i = firstBooleanParameterIndex; i < types.length; i++) {
						if (i == firstBooleanParameterIndex) {
							...
							if (types[i].getDescriptor().equals("Z")
								...
								MethodInsnNode booleanHelperCast = new MethodInsnNode(Opcodes.INVOKESTATIC,
										Type.getInternalName(BooleanHelper.class), "RevertIntToBoolean",
										Type.getMethodDescriptor(Type.BOOLEAN_TYPE, new Type[] { Type.INT_TYPE }));
			...
			// If it's a non-boolean method, make sure there are no boolean
			// parameters
			...
			/*
			 * Type[] types = Type.getArgumentTypes(node.desc); try { Analyzer a
			 * = new Analyzer(new IsBooleanInterpreter()); a.analyze(cn.name,
			 * mn); Frame[] frames = a.getFrames(); Frame current =
			 * frames[mn.instructions.indexOf(node)]; int
			 * firstBooleanParameterIndex = -1; int stackSize =
			 * current.getStackSize(); for (int i = 0; i < types.length; i++) {
			 * if (current.getStack(stackSize - types.length + i) ==
			 * IsBooleanInterpreter.BOOLEAN) { firstBooleanParameterIndex = i;
			 * break; } } if (firstBooleanParameterIndex > 0) {
			 * logger.info("Need to check starting with argument " +
			 * firstBooleanParameterIndex + " out of " + types.length); int
			 * numOfPushs = types.length - 1 - firstBooleanParameterIndex; if
			 * (numOfPushs == 0) { //the boolean parameter is the last parameter
			 * MethodInsnNode booleanHelperInvoke = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "booleanToInt", Type.getMethodDescriptor(Type.INT_TYPE, new
			 * Type[] { Type.BOOLEAN_TYPE }));
			 * mn.instructions.insertBefore(node, booleanHelperInvoke); }else {
			 * InsnList insnlist = new InsnList();
			 * 
			 * for (int i = 0; i < numOfPushs; i++) { MethodInsnNode
			 * booleanHelperPushParameter; if (types[types.length - 1 - i] ==
			 * Type.BOOLEAN_TYPE || types[types.length - 1 - i] ==
			 * Type.CHAR_TYPE || types[types.length - 1 - i] == Type.BYTE_TYPE
			 * || types[types.length - 1 - i] == Type.SHORT_TYPE ||
			 * types[types.length - 1 - i] == Type.INT_TYPE ||
			 * types[types.length - 1 - i] == Type.FLOAT_TYPE ||
			 * types[types.length - 1 - i] == Type.LONG_TYPE ||
			 * types[types.length - 1 - i] == Type.DOUBLE_TYPE) { if
			 * (types[types.length - 1 - i] == Type.BOOLEAN_TYPE) {
			 * booleanHelperPushParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "pushParameter", Type.getMethodDescriptor(Type.VOID_TYPE, new
			 * Type[] { Type.BOOLEAN_TYPE })); } else {
			 * booleanHelperPushParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "pushParameter", Type.getMethodDescriptor(Type.VOID_TYPE, new
			 * Type[] { types[types.length - 1 - i] })); } } else {
			 * booleanHelperPushParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "pushParameter", Type.getMethodDescriptor(Type.VOID_TYPE, new
			 * Type[] { Type.getType(Object.class) })); }
			 * 
			 * insnlist.add(booleanHelperPushParameter); } for (int i =
			 * firstBooleanParameterIndex; i < types.length; i++) { if (i ==
			 * firstBooleanParameterIndex) { MethodInsnNode booleanHelperInvoke
			 * = new MethodInsnNode( Opcodes.INVOKESTATIC,
			 * Type.getInternalName(BooleanHelper.class), "booleanToInt",
			 * Type.getMethodDescriptor(Type.INT_TYPE, new Type[] {
			 * Type.BOOLEAN_TYPE })); insnlist.add(booleanHelperInvoke); } else
			 * { MethodInsnNode booleanHelperPopParameter; boolean
			 * objectNeedCast = false; if (types[i] == Type.BOOLEAN_TYPE) {
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterBooleanFromInt", Type.getMethodDescriptor(types[i],
			 * new Type[] {})); } else if (types[i] == Type.CHAR_TYPE) {
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterChar", Type.getMethodDescriptor(types[i], new Type[]
			 * {})); } else if (types[i] == Type.BYTE_TYPE) {
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterByte", Type.getMethodDescriptor(types[i], new Type[]
			 * {})); } else if (types[i] == Type.SHORT_TYPE) {
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterShort", Type.getMethodDescriptor(types[i], new
			 * Type[] {})); } else if (types[i] == Type.INT_TYPE) {
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterInt", Type.getMethodDescriptor(types[i], new Type[]
			 * {})); } else if (types[i] == Type.FLOAT_TYPE) {
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterFloat", Type.getMethodDescriptor(types[i], new
			 * Type[] {})); } else if (types[i] == Type.LONG_TYPE) {
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterLong", Type.getMethodDescriptor(types[i], new Type[]
			 * {})); } else if (types[i] == Type.DOUBLE_TYPE) {
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterDouble", Type.getMethodDescriptor(types[i], new
			 * Type[] {})); } else { objectNeedCast = true;
			 * booleanHelperPopParameter = new MethodInsnNode(
			 * Opcodes.INVOKESTATIC, Type.getInternalName(BooleanHelper.class),
			 * "popParameterObject",
			 * Type.getMethodDescriptor(Type.getType(Object.class), new Type[]
			 * {})); }
			 * 
			 * insnlist.add(booleanHelperPopParameter); if (objectNeedCast) {
			 * TypeInsnNode tin = new TypeInsnNode(Opcodes.CHECKCAST,
			 * types[i].getInternalName()); insnlist.add(tin); } if
			 * (types[i].getDescriptor().equals("Z") ||
			 * types[i].getDescriptor().
			 * equals(Type.getDescriptor(Boolean.class))) { MethodInsnNode
			 * booleanHelperCast = new MethodInsnNode( Opcodes.INVOKESTATIC,
			 * Type.getInternalName(BooleanHelper.class), "RevertIntToBoolean",
			 * Type.getMethodDescriptor(Type.BOOLEAN_TYPE, new Type[] {
			 * Type.INT_TYPE })); insnlist.add(booleanHelperCast); } }
			 * 
			 * } mn.instructions.insertBefore(node, insnlist); } }
			 * 
			 * } catch (AnalyzerException e) { logger.error("Analysis failed: "
			 * + e); }
			 */
		}
	}
				...
				if (min.owner.equals("java/lang/String")) {
					if (min.name.equals("equals")) {
						MethodInsnNode equalCheck = new MethodInsnNode(Opcodes.INVOKESTATIC,
								Type.getInternalName(BooleanHelper.class), "StringEquals", Type.getMethodDescriptor(
										Type.INT_TYPE,
						...
						MethodInsnNode equalCheck = new MethodInsnNode(Opcodes.INVOKESTATIC,
								Type.getInternalName(BooleanHelper.class), "StringEndsWith", Type.getMethodDescriptor(
										Type.INT_TYPE,
										new Type[] { Type.getType(String.class), Type.getType(String.class) }));
						mn.instructions.insertBefore(node, equalCheck);
						mn.instructions.remove(node);
						...
						MethodInsnNode equalCheck = new MethodInsnNode(Opcodes.INVOKESTATIC,
								Type.getInternalName(BooleanHelper.class), "StringIsEmpty", Type.getMethodDescriptor(
										Type.INT_TYPE, new Type[] { Type.getType(String.class) }));
						mn.instructions.insertBefore(node, equalCheck);
						mn.instructions.remove(node);
