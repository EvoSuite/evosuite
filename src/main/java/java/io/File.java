...
public class File implements Serializable, Comparable<File> {
	...
	/**
	 * indicates, if this file shall be handled like an 'original' file (in its
	 * original implementation)
	 */
	private boolean isOriginal = false;

	/**
	 * the correspondent memory representation of this file (may be
	 * <code>null</code>)
	 */
	private FileObject ramFile;

	/**
	 * a global map to keep track of file permissions; the mapping consists of
	 * canonicalPath -> permission (binary flag)
	 */
	...
	/**
	 * returns the correspondent memory representation of this file; creates it,
	 ...
	 * file is readable, then it is copied into ram; if the real file is not
	...
	private void initRamFile() throws IOException {
		...
		if (existsOriginal() && !ramFile.exists()) {
			...
			if (canReadOriginal())
				permissionBitstring = permissionBitstring | FileSystem.ACCESS_READ;
			if (canWriteOriginal())
				permissionBitstring = permissionBitstring | FileSystem.ACCESS_WRITE;
			if (canExecuteOriginal())
				permissionBitstring = permissionBitstring | FileSystem.ACCESS_EXECUTE;
	...
	public boolean setWritable(boolean writable, boolean ownerOnly) {
		if (!isOriginal && USE_SIMULATION) {
			try {
				...
				int permissionBitstring = 0x00;
				if (permissions.containsKey(canonicalPath)) {
					permissionBitstring = permissions.get(canonicalPath).intValue(); // TODO test unboxing
				}
				if (writable) { // we do not care about the ownerOnly flag, since we access the file anyway only as the owner
					permissionBitstring = permissionBitstring | FileSystem.ACCESS_WRITE; // set the write bit
				} else {
					permissionBitstring = permissionBitstring & ~FileSystem.ACCESS_WRITE; // unset the write bit
				}
				permissions.put(canonicalPath, permissionBitstring);
			} catch (IOException e) {
				if (e instanceof FileSystemException) {
		...
		if (!isOriginal && USE_SIMULATION) {
			try {
				...
				if (readable) { // we do not care about the ownerOnly flag, since we access the file anyway only as the owner
					permissionBitstring = permissionBitstring | FileSystem.ACCESS_READ; // set the write bit
				} else {
					permissionBitstring = permissionBitstring & ~FileSystem.ACCESS_READ; // unset the write bit
				}
				permissions.put(canonicalPath, permissionBitstring);
			} catch (IOException e) {
				if (e instanceof FileSystemException) {
		...
		if (!isOriginal && USE_SIMULATION) {
			try {
				...
				if (executable) { // we do not care about the ownerOnly flag, since we access the file anyway only as the owner
					permissionBitstring = permissionBitstring | FileSystem.ACCESS_EXECUTE; // set the execute bit
				} else {
	...
	private static class LazyInitialization {
		...
		static String temporaryDirectory() {
			return fs.normalize(AccessController.doPrivileged(new GetPropertyAction(
			        "java.io.tmpdir")));
		}
	}
	...
	/**
	 * @return true, if the file shall be treated with the original
	 *         implementation; false, if otherwise
	 */
	...
	/**
	 * @param isOriginal
	 *            true, if the file shall be treated with the original
	 *            implementation; false, if otherwise
	 */
