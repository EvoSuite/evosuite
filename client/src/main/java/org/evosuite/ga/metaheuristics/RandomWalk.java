...
public class RandomWalk<T extends Chromosome> extends GeneticAlgorithm<T> {
    ...
    protected void evolve() {
        ...
        getFitnessFunctions().forEach(f -> fitnessValueCache.put(Pair.of(f, currentIteration), f.getFitness(newChromosome)));
        currentIteration++;
    }
    ...
    private Pair<Integer, List<Integer>> computeNV(List<Double> landscape) {
        int result = 0;
        for (int i = 1; i < landscape.size(); i++) {
            if (!landscape.get(i).equals(landscape.get(i - 1)))
                result++;
        }
            ...
            for (int j = -1; j <= 1; j++) {
                if (i != j) {
                    ...
                    if (count > 0) {
                        ...
                        sum += freq * log;
                        if (Double.isNaN(sum))
                            throw new IllegalStateException("IC is NaN now " + freq + " " + log);
                    }
                }
            }
        }
    ...
    private void computeFitnessLandscapeMeasurements() throws IOException {
        List<? extends FitnessFunction<?>> orderInBitString = new BranchCoverageFactory().getCoverageGoals();
        ...
        for (FitnessFunction<T> fitnessFunction : fitnessFunctions) {
            ...
            if (nvLandscape.size() == 0) {
                logger.warn(String.format("%d: zero size nvLandscape, actual landscape = %s ", id, landscape.toString()));
            }
            double ic = computeIC(nvLandscape);
            double pic = computePIC(nvLandscape);
            double dbi = computeDBI(nvLandscape);
            csv.add(String.format("%d,%s,%.5f,%d,%d,%.5f,%.5f,%.5f,%d,%s,%d,%d,%d",
                    id++,
            ...
            if (fitnessFunction instanceof BranchCoverageTestFitness) {
                ...
                if(branch != null) {
                    ...
                    if (isSwitch) {
                        ...
                        targetCaseValue = targetCaseValue1 != null ? targetCaseValue1 :
                                -1;
                    } else {
                        targetCaseValue = -1;
                    }
                ...
                csv.add(String.format("%s,%s,%d,%d,%d,%d,%s,%s,%d",
                        branchGoal.getClassName(),
                        branchGoal.getMethodName(),
                        i,
                        instructionId,
                        actualBranchId,
                        lineNumber,
