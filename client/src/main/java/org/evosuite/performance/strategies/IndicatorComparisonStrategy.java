...
public class IndicatorComparisonStrategy<T extends Chromosome> implements PerformanceStrategy<T> {
    ...
    public void setDistances(List<T> front) {
        ...
        for(String indicator : ((TestChromosome)front.get(0)).getIndicators()){
            logger.debug("Indicator = {}", indicator);
            comparator.setIndicator(indicator);
            Double min = Collections.min(front, comparator).getIndicatorValue(indicator);
            Double max = Collections.max(front, comparator).getIndicatorValue(indicator);

            for (T individual : front) {
                if (Double.compare(min, max) != 0) {
                    double currentValue = individual.getIndicatorValue(indicator);
                    double normalizedValue = (max - currentValue) / (max - min);
                    logger.debug("{}, {}, {}, {} >> {}", indicator, min, max, currentValue, normalizedValue);
                    ...
                    logger.debug("Same min and max while computing the value for {}" + indicator);
