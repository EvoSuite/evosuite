    ...
    public static PerformanceStrategy getPerformanceStrategy() {
        switch (Properties.P_STRATEGY) {
            case CROWDING_DISTANCE:
                LoggingUtils.getEvoLogger().info("* Running Performance MOSA with CROWDING DISTANCE strategy");
                return new IndicatorComparisonStrategy();
            case PREFERENCE_CRITERION:
                LoggingUtils.getEvoLogger().info("* Running Performance MOSA with PREFERENCE CRITERION strategy");
