...
public abstract class AbstractInsertion implements InsertionStrategy {

    private static final Logger logger = LoggerFactory.getLogger(AbstractInsertion.class);
    ...
    public int insertStatement(TestCase test, int lastPosition) {
        ...
        if (insertUUT) {
            position = test.size();
            success = insertUUT(test, lastPosition + 1);
        } else if (insertEnv) {
            ...
            position = insertRandomCallOnEnvironment(test, lastPosition);
            success = (position >= 0);
        } else {
            /*
...
//                    success = insertRandomCall(test, position);
    ...
    /**
     * Inserts a call (or possibly multiple calls) to the UUT into the given test case {@code test}
     * at the specified {@code position}.
     *
    ...
    protected abstract int insertParam(TestCase test, int lastPosition);
     ...
     */
    private int insertRandomCallOnEnvironment(TestCase test, int lastValidPosition) {
     ...
     */
    protected boolean insertRandomCall(TestCase test, int position) {
        final GenericAccessibleObject<?> o;
        try {
            logger.debug("Choosing random call");
            o = testCluster.getRandomTestCall(test);
        } catch (ConstructionFailedException e) {
    ...
    protected int insertRandomParam(TestCase test, int lastPosition) {
    ...
    public boolean insertRandomCallOnObjectAt(TestCase test, VariableReference var, int position) {
        ...
        if (var instanceof ArrayReference) {
            ...
            try {
                ...
                GenericAccessibleObject<?> gao = testCluster.getRandomCallFor(var.getGenericClass(), test, position);
     ...
     * @throws ConstructionFailedException if the maximum recursion depth has been reached
     */
    private VariableReference addConstructor(TestCase test,
    ...
    private VariableReference addFunctionalMock(TestCase test, Type type, int position, int recursionDepth)
    ...
    private VariableReference addFunctionalMockForAbstractClass(TestCase test, Type type, int position, int recursionDepth)
            throws ConstructionFailedException, IllegalArgumentException {
    ...
    private VariableReference createVariable(TestCase test, Type parameterType,
        ...
        if (clazz.isEnum() || clazz.isPrimitive() || clazz.isWrapperType() || clazz.isObject() ||
                ...
                clazz.isString() || clazz.isArray() || testCluster.hasGenerator(parameterType) ||
    ...
    private VariableReference createObject(TestCase test, Type type, int position,
        ...
        if (canUseFunctionalMocks && TimeController.getInstance().getPhasePercentage() >= Properties.FUNCTIONAL_MOCKING_PERCENT &&
            ...
            GenericAccessibleObject<?> o = testCluster.getRandomGenerator(
...
//                        Class<?> mock = MockList.getMockClass(clazz.getRawClass().getCanonicalName());
     ...
     * @throws ConstructionFailedException
     */
    private void assignArray(TestCase test, VariableReference array, int arrayIndex,
    ...
    protected boolean insertCallFor(TestCase test, GenericAccessibleObject<?> o, int position) {
        int previousLength = test.size();
        String name = "";
        currentRecursion.clear();
        ...
        try {
            ...
            if (reflectionFactory.hasPrivateFieldsOrMethods() &&
                    TimeController.getInstance().getPhasePercentage() >= Properties.REFLECTION_START_PERCENT &&
                    (Randomness.nextDouble() < Properties.P_REFLECTION_ON_PRIVATE || testCluster.getNumTestCalls() == 0)) {
...
//					!var.getVariableClass().equals(Properties.getTargetClass()) &&
     ...
     * @throws ConstructionFailedException if creation fails
     */
    private VariableReference attemptInstantiationOfObjectClass(TestCase test, int position,
        ...
        List<GenericClass> classes = castClasses.stream()
                .filter(c -> testCluster.hasGenerator(c) || c.isString())
        ...
        GenericAccessibleObject<?> o = testCluster.getRandomGenerator(choice);
        currentRecursion.add(o);

        if (o == null) {

            if (!testCluster.hasGenerator(Object.class)) {
