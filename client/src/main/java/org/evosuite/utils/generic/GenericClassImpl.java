...
//                logger.warn(type + " points to itself in type mapping");
    ...
    private GenericClass<?> getGenericTypeVariableInstantiation(
            ...
            throws ConstructionFailedException {
//        if(this.getType() instanceof TypeVariableImpl) {
//            if(((TypeVariableImpl<?>) this.getType()).getGenericDeclaration().toString().contains("CtOperatorAssignmentImpl")) {
//                logger.debug("time to track the bug");
//            }
//        }
        ...
        for (GenericClass<?> parameterClass : getParameterClasses()) {
            ...
            if (!parameterClass.hasWildcardOrTypeVariables()) {
                ...
                if (parameterClass.isWildcardType()) {
                    ...
                    if(recursionLevel >= 1) {
                        // List<List<String>> cannot be assigned to List<List<?>>
                        parameterTypes[numParam++] = parameterClass.getType();
                    }else {
...
//                    if(numParam == 1) {
//                        if(parameterTypes[numParam] instanceof ParameterizedType) {
//                            if(((ParameterizedType) parameterTypes[numParam]).getRawType().toString().contains("CtOperatorAssignmentImpl")) {
