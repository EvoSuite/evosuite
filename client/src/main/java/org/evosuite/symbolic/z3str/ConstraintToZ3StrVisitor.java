...
class ConstraintToZ3StrVisitor implements ConstraintVisitor<String, Void> {
	...
	private String mkComparison(String left, Comparator cmp,
		...
		switch (cmp) {
		case LT: {
			String lt = Z3ExprBuilder.mkLt(left, right);
			return lt;
		}
		case LE: {
			String le = Z3ExprBuilder.mkLe(left, right);
			return le;
		}
		case GT: {
			String gt = Z3ExprBuilder.mkGt(left, right);
			return gt;
		}
		case GE: {
			String ge = Z3ExprBuilder.mkGe(left, right);
			return ge;
		}
		case EQ: {
			String ge = Z3ExprBuilder.mkEq(left, right);
			return ge;
		}
		case NE: {
			String ge = Z3ExprBuilder.mkEq(left, right);
