...
public class CoverageGoalTestNameGenerationStrategy implements TestNameGenerationStrategy {
	...
	private void generateNames(Map<TestCase, Set<TestFitnessFunction>> testToGoals) {
		...
		while (changed) {
			...
			for (Map.Entry<String, Set<TestCase>> entry : testNameMap.entrySet()) {
				...
				if (resolveAmbiguity(testToGoals, entry.getValue(), true))
					...
					// If there is nothing unique, try using something that
					// reduces
		...
		// If there is absolutely nothing unique, add the top goals so that the
		// test at least has a name
		for (Map.Entry<TestCase, Set<TestFitnessFunction>> entry : testToGoals.entrySet()) {
	...
	/**
	 * There is nothing unique about the test, so we have to add goals until we
	 * find a unique name
	 */
	...
	/**
	 * Add additional goals for tests to testsToGoals to make them
	 * distinguishable
	 *
	 * @param testToGoals
	 * @param tests
	...
	private boolean resolveAmbiguity(Map<TestCase, Set<TestFitnessFunction>> testToGoals, Set<TestCase> tests,
		...
		for (TestCase test : tests) {
			...
			goals.removeAll(testToGoals.get(test)); // Remove those already used
													// to name the test
			fullTestToGoals.put(test, goals);
		}

		// Find out what is unique about each one, or at least helps reduce the
		// number of ambiguous names
		if (unique)
		...
		for (TestCase test : tests) {
			...
			if (topGoals.isEmpty()) {
				...
				String newName = testToName.get(test); // getTestName(test,
														// newGoals);
	...
	/**
	 * Builds the name map based on coverage goal stored as covered in each of
	 * the tests
	 * 
	 * @param tests
	 * @return
	...
	/**
	 * Builds the name map based on coverage goals stored as covered in the
	 * tests pointed to by results
	 * 
	 * @param results
	 * @return
	 */
	...
	/**
	 * Name generation assumes that certain coverage criteria are included. If
	 * we haven't targeted them yet, we need to determine the covered goals.
	 * This may require re-executing the tests with observers.
	 *
	 * @param results
	 */
	private void addGoalsNotIncludedInTargetCriteria(List<ExecutionResult> results) {
		List<Properties.Criterion> requiredCriteria = new ArrayList<>(Arrays.asList(new Properties.Criterion[] {
				Properties.Criterion.OUTPUT, Properties.Criterion.INPUT, Properties.Criterion.METHOD,
				Properties.Criterion.METHODNOEXCEPTION, Properties.Criterion.EXCEPTION }));
	...
	/**
	 * Some criteria require re-execution with observers. Make sure the results
	 * are up-to-date
	 *
	...
	private List<Class<?>> supportedClasses = Arrays.asList(new Class<?>[] { MethodCoverageTestFitness.class,
			MethodNoExceptionCoverageTestFitness.class, ExceptionCoverageTestFitness.class,
			OutputCoverageTestFitness.class, InputCoverageTestFitness.class });
	...
	/**
	 * Determine if we have overloaded methods, which requires the use of
	 * signatures
	 *
	 * @param testToGoals
	 */
	...
	/**
	 * Determine for each test the set of coverage goals uniquely covered by
	 * this test
	 *
	 * @param testToGoals
	 */
	...
	/**
	 * Determine for each test the set of coverage goals that remain if we
	 * remove those that are covered by all tests
	 *
	 * @param testToGoals
	 */
	...
	/**
	 * There may be tests with the same calculated name, in which case we add a
	 * number suffix
	 */
	...
	private String getTestName(TestCase test, Set<TestFitnessFunction> uniqueGoals) {
		...
		if (goalList.isEmpty()) {
			...
			// name += capitalize(getGoalName(chooseRepresentativeGoal(test,
			// goalList)));
		}
		return name;
	}
	 ...
	 * Assumes that goals is not empty, and all items in goals have the same
	 * type
	 * 
	 * @param test
	 * @param goals
	...
	private String getGoalName(ExceptionCoverageTestFitness goal) {
		...
		if (goal.getTargetMethod().startsWith("<init>")) {
			return STR_CREATE_EXCEPTION
					+ capitalize(getUniqueConstructorName(goal.getTargetClass(), goal.getTargetMethod())) + STR_THROWS
					+ capitalize(ex.getSimpleName());
		}
		return formatMethodName(goal.getTargetClass(), goal.getTargetMethod()) + STR_THROWS
				+ capitalize(ex.getSimpleName());
	}
	...
	private String formatValueDescriptor(String descriptor) {
		...
		if (components.length == 1) {
			...
			// Inspector
			return capitalize(getShortClassName(components[1])) + STR_WHERE
					+ capitalize(getMethodNameWithoutDescriptor(components[2])) + STR_IS + capitalize(components[3]);
		} else {
	...
	private String getGoalPairName(TestFitnessFunction goal1, TestFitnessFunction goal2) {
		if (goal1.getClass().equals(goal2.getClass())) {
			...
			if (goal1.getTargetClass().equals(goal2.getTargetClass())
					&& goal1.getTargetMethod().equals(goal2.getTargetMethod())) {
	...
	private String getGoalPairName(InputCoverageTestFitness goal1, InputCoverageTestFitness goal2) {
		return formatMethodName(goal1.getClassName(), goal1.getMethod()) + STR_WITH
				+ formatValueDescriptor(goal1.getValueDescriptor()) + STR_AND
				+ formatValueDescriptor(goal2.getValueDescriptor());
	}
	...
	private String getGoalPairName(OutputCoverageTestFitness goal1, OutputCoverageTestFitness goal2) {
		return formatMethodName(goal1.getClassName(), goal1.getMethod()) + STR_RETURNS
				+ formatValueDescriptor(goal1.getValueDescriptor()) + STR_AND
				+ formatValueDescriptor(goal2.getValueDescriptor());
	}

	/**
	 * Make sure package names are omitted and array brackets are not used in
	 * names
	 * 
	 * @param className
	 * @return
	...
	private String getUniqueMethodName(String methodNameWithoutDescriptor, String methodName) {
		...
		if (argumentTypes.length == 0)
			...
			return methodNameWithoutDescriptor + STR_TAKING
					+ capitalize(getShortClassName(argumentTypes[0].getClassName()));
		} else
			return methodNameWithoutDescriptor + STR_TAKING + argumentTypes.length + STR_ARGUMENTS;
	}

	/**
	 * Get a unique constructor name, depending on whether it is unique or
	 * overloaded
	 * 
	 * @param className
	 * @param methodName
	...
	private String getUniqueConstructorName(String className, String methodName) {
		...
		int pos = methodName.indexOf('(');
		if (pos < 0) {
			return getShortClassName(className); // TODO: Should this really be
													// possible?
		}
		String descriptor = methodName.substring(pos);
		Type[] argumentTypes = Type.getArgumentTypes(descriptor);
		...
		if (argumentTypes.length == 0)
			...
			return getShortClassName(className) + STR_TAKING
					+ capitalize(getShortClassName(argumentTypes[0].getClassName()));
		} else
