...
public class HardBenchmarkTestClass {
	...
	/**
	 * Supposed to show that you might have to follow an arbitrarily complex
	 * path in order to not block the targetUse by another def.
	 * 
	 * Or in other words: Satisfying a DefUse-Pair is as hard as satisfying a
	 * specific path.
	 * 
	 * ... turns out this also shows that covering a certain BranchCoverageGoal
	 * can be that hard :D
	 */
	public void mean() {
		...
		else if ((yetAnotherField != someField + someOtherField)
				|| yetAnotherField == 0)
			targetField = 3;
		else {
			...
			if (auxField0 == 0 || auxField0 != yetAnotherField * 3)
				...
				if (auxField4 == 0 || auxField4 != auxField3 + 2)
					...
					targetField = 11;
						...
						|| auxField8 != auxField7 + 3 * auxField2 + 13)
					targetField = 12;
				else if (auxField9 == 0
						|| auxField9 != auxField8 - 13 * auxField1 * auxField6)
					targetField = 13;
			}
		}
