...
def getEvoSuiteCall(seed, configId, config, project, clazz, id, strategy, coreIndex):
  ...
  if ALGORITHM == 'PDMOSA':
    ...
    result += ' -Dconfiguration_id=dynamosa'
  
  # parameters in common
  result += " -projectCP " + class_path
  result += ' -Dperformance_indicators=LOOP_COUNTER:METHOD_CALL:COVERED_METHOD_CALL:STATEMENTS_COUNTER:STATEMENTS_COVERED:OBJECTS_INSTANTIATIONS'
  result += ' -Doutput_variables=TARGET_CLASS,criterion,configuration_id,Total_Goals,Total_Branches,Lines,Covered_Goals,Generations,Statements_Executed,Fitness_Evaluations,Tests_Executed,Generations,Total_Time,Size,Length,BranchCoverage,ObjectsInstantiations,StatementCovered,StatementCounter,MethodCall,CoveredMethodCall,LoopCounter,TestExecutionTime,Random_Seed'
