...
public class JDK17SystemTestBase {

    protected static final String JDK17_TEST_PACKAGE_PATH = "com.examples.with.different.packagename.jdk17";
    ...
    public void setDefaultPropertiesForTestCases() {
        ...
        Properties.SERIALIZE_GA = true;
        Properties.ASSERTIONS = true;

        Properties.SAVE_SOLUTION_IN_RESULTBUILDER = true;
...
//		long seed = getSeed();
    ...
    protected <T extends Chromosome<T>, U extends TestSuiteChromosome> U getTestSuiteFromResult(Object result) {
        assert (result instanceof List);
        List<List<TestGenerationResult<T>>> results = (List<List<TestGenerationResult<T>>>) result;
        assert (results.size() == 1);
        return (U) results.get(0).get(0).getTestSuite();
    ...
    // NOTE: Since EvoSuite is compiled with JDK8, classes compiled with JDK17 should be referred by their path not by import
    protected String getJDK17CUTCanonicalName(String className) {
        return JDK17_TEST_PACKAGE_PATH + "." + className;
    }
