...
# Constants
readonly PROJECTS_DIRECTORY='projects' # Directory where the projects are stored
...
get_project_class_path() {
  ...
  local search_depth=${2:-1}

  # Find all paths of jars in the search directory and combine with a colon separator
  local _project_class_path="$(find "${PROJECTS_DIRECTORY}"/"${project_name}" -maxdepth "${search_depth}" -type f -name "*.jar" -printf '%p:' | sed 's/:$//')"

  echo $_project_class_path
}
